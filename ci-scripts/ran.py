...
#-----------------------------------------------------------
	...
	def InitializeeNB(self, HTML, EPC):
		...
		mySSH.command('echo "ulimit -c unlimited && export LD_LIBRARY_PATH=./ran_build/build && ./ran_build/build/' + self.air_interface[self.eNB_instance] + ' -O ' + lSourcePath + '/' + ci_full_config_file + extra_options + '" > ./my-lte-softmodem-run' + str(self.eNB_instance) + '.sh', '\$', 5)
