...
static void init_NR_SI(gNB_RRC_INST *rrc) {
  ...
  if (get_softmodem_params()->phy_test > 0 || get_softmodem_params()->do_ra > 0) {
    ...
    if (fd != NULL) {
      ...
      NR_UL_DCCH_Message_t *NR_UL_DCCH_Message;
      asn_dec_rval_t dec_rval = uper_decode_complete( NULL,
                                &asn_DEF_NR_UL_DCCH_Message,
                                (void **)&NR_UL_DCCH_Message,
                                (uint8_t *)buffer,
                                msg_len);
      ...
      if ((dec_rval.code != RC_OK) && (dec_rval.consumed == 0)) {
        ...
        // free the memory
        SEQUENCE_free( &asn_DEF_NR_UL_DCCH_Message, NR_UL_DCCH_Message, 1 );
        return;
      }

      fclose(fd);
      xer_fprint(stdout,&asn_DEF_NR_UL_DCCH_Message, NR_UL_DCCH_Message);
      // recreate enough of X2 EN-DC Container
      AssertFatal(NR_UL_DCCH_Message->message.choice.c1.present == NR_UL_DCCH_MessageType__c1_PR_ueCapabilityInformation,
      ...
      asn_enc_rval_t enc_rval = uper_encode_to_buffer(&asn_DEF_NR_UE_CapabilityRAT_ContainerList,NULL,
                                (void *)&NR_UL_DCCH_Message->message.choice.c1.choice.ueCapabilityInformation.criticalExtensions.choice.c1.choice.ueCapabilityInformation_r8.ue_CapabilityRAT_ContainerList,buffer,4096);
...
static struct rrc_gNB_ue_context_s * rrc_gNB_ue_context_random_exist(
  const protocol_ctxt_t *const ctxt_pP,
...
static struct rrc_gNB_ue_context_s * rrc_gNB_ue_context_stmsi_exist(
  const protocol_ctxt_t *const ctxt_pP,
...
static struct rrc_gNB_ue_context_s * rrc_gNB_get_next_free_ue_context(
  const protocol_ctxt_t *const ctxt_pP,
  const uint64_t         ue_identityP)
{
  struct rrc_gNB_ue_context_s  *ue_context_p = NULL;
  ...
  if (ue_context_p == NULL) {
    ue_context_p = rrc_gNB_allocate_new_UE_context(RC.rrc[ctxt_pP->module_id]);
  ...
  struct rrc_gNB_ue_context_s         *const ue_context_pP)
{
  ...
  LOG_T(RRC,
        ...
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        ue_context_pP);
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_NR_SCellToAddMod_r10, &ue_context_pP->ue_context.sCell_config[0]);
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_NR_SCellToAddMod_r10, &ue_context_pP->ue_context.sCell_config[1]);

  if (ue_context_pP->ue_context.SRB_configList) {
    ASN_STRUCT_FREE(asn_DEF_NR_SRB_ToAddModList, ue_context_pP->ue_context.SRB_configList);
  ...
  if (ue_context_pP->ue_context.DRB_configList) {
    ASN_STRUCT_FREE(asn_DEF_NR_DRB_ToAddModList, ue_context_pP->ue_context.DRB_configList);
  ...
  if (ue_context_pP->ue_context.physicalConfigDedicated) {
    ASN_STRUCT_FREE(asn_DEF_NR_PhysicalConfigDedicated, ue_context_pP->ue_context.physicalConfigDedicated);
    ue_context_pP->ue_context.physicalConfigDedicated = NULL;
  }

  if (ue_context_pP->ue_context.sps_Config) {
    ASN_STRUCT_FREE(asn_DEF_NR_SPS_Config, ue_context_pP->ue_context.sps_Config);
    ue_context_pP->ue_context.sps_Config = NULL;
  }

  for (i=0; i < MAX_MEAS_OBJ; i++) {
    if (ue_context_pP->ue_context.MeasObj[i] != NULL) {
      ASN_STRUCT_FREE(asn_DEF_NR_MeasObjectToAddMod, ue_context_pP->ue_context.MeasObj[i]);
  ...
  for (i=0; i < MAX_MEAS_CONFIG; i++) {
    if (ue_context_pP->ue_context.ReportConfig[i] != NULL) {
      ASN_STRUCT_FREE(asn_DEF_NR_ReportConfigToAddMod, ue_context_pP->ue_context.ReportConfig[i]);
      ue_context_pP->ue_context.ReportConfig[i] = NULL;
    }
  }

  if (ue_context_pP->ue_context.QuantityConfig) {
    ASN_STRUCT_FREE(asn_DEF_NR_QuantityConfig, ue_context_pP->ue_context.QuantityConfig);
    ue_context_pP->ue_context.QuantityConfig = NULL;
  }

  if (ue_context_pP->ue_context.mac_MainConfig) {
    ASN_STRUCT_FREE(asn_DEF_NR_MAC_MainConfig, ue_context_pP->ue_context.mac_MainConfig);
    ue_context_pP->ue_context.mac_MainConfig = NULL;
  }

  /*  if (ue_context_pP->ue_context.measGapConfig) {
      ASN_STRUCT_FREE(asn_DEF_NR_MeasGapConfig, ue_context_pP->ue_context.measGapConfig);
  ...
  if (ue_context_pP->ue_context.measConfig) {
    ASN_STRUCT_FREE(asn_DEF_NR_MeasConfig, ue_context_pP->ue_context.measConfig);
...
//-----------------------------------------------------------------------------
//TODO
int rrc_gNB_previous_SRB2(rrc_gNB_ue_context_t         *ue_context_pP) {
  struct NR_SRB_ToAddMod                *SRB2_config = NULL;
  uint8_t i;
  NR_SRB_ToAddModList_t                 *SRB_configList = ue_context_pP->ue_context.SRB_configList;
  NR_SRB_ToAddModList_t                **SRB_configList2 = &ue_context_pP->ue_context.SRB_configList2[ue_context_pP->ue_context.reestablishment_xid];

  if (*SRB_configList2 != NULL) {
    if((*SRB_configList2)->list.count!=0) {
      LOG_D(RRC, "rrc_gNB_previous_SRB2 SRB_configList2(%p) count is %d\n           SRB_configList2->list.array[0] addr is %p",
    ...
    LOG_E(RRC, "rrc_gNB_previous_SRB2 SRB_configList2 NULL\n");
  }

  if (SRB2_config != NULL) {
    ASN_SEQUENCE_ADD(&SRB_configList->list, SRB2_config);
  } else {
    LOG_E(RRC, "rrc_gNB_previous_SRB2 SRB2_config NULL\n");
  }
...
//TODO
//-----------------------------------------------------------------------------
void rrc_gNB_generate_RRCReestablishmentReject(
  const protocol_ctxt_t *const ctxt_pP,
  rrc_gNB_ue_context_t  *const ue_context_pP,
  const int              CC_id)
  {
  if (!NODE_IS_CU(RC.rrc[ctxt_pP->module_id]->node_type)) {
    ...
    if(UE_id != -1) {
      ...
      LOG_E(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" Generating NR_RRCReestablishmentReject without UE_id(MAC) rnti %x\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),ctxt_pP->rnti);
    }
  }
  //T(T_ENB_RRC_REESTABLISHMENT_REJECT, T_INT(ctxt_pP->module_id), T_INT(ctxt_pP->frame),
   // T_INT(ctxt_pP->subframe), T_INT(ctxt_pP->rnti));
  gNB_RRC_UE_t *ue_p = &ue_context_pP->ue_context;
  //TODO
  ue_p->Srb0.Tx_buffer.payload_size =
    do_RRCReestablishmentReject(ctxt_pP->module_id,
                                          (uint8_t *) ue_p->Srb0.Tx_buffer.Payload);
  LOG_DUMPMSG(RRC,DEBUG_RRC,
              (char *)(ue_p->Srb0.Tx_buffer.Payload),
              ue_p->Srb0.Tx_buffer.payload_size,
              "[MSG] RRCReestablishmentReject\n");
  MSC_LOG_TX_MESSAGE(
    MSC_RRC_ENB,
    MSC_RRC_UE,
    ue_p->Srb0.Tx_buffer.Header,
    ue_p->Srb0.Tx_buffer.payload_size,
    MSC_AS_TIME_FMT" NR_RRCReestablishmentReject UE %x size %u",
    MSC_AS_TIME_ARGS(ctxt_pP),
    ue_context_pP == NULL ? -1 : ue_context_pP->ue_context.rnti,
    ue_p->Srb0.Tx_buffer.payload_size);
  LOG_I(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT" [RAPROC] Logical Channel DL-CCCH, Generating NR_RRCReestablishmentReject (bytes %d)\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        ue_p->Srb0.Tx_buffer.payload_size);
}
  ...
  const int              CC_id)
{
  ...
  //TODO asn1?
  ue_p->Srb0.Tx_buffer.payload_size =
    do_RRCReject(ctxt_pP->module_id,
                           (uint8_t *) ue_p->Srb0.Tx_buffer.Payload);
  LOG_DUMPMSG(RRC,DEBUG_RRC,
              (char *)(ue_p->Srb0.Tx_buffer.Payload),
              ue_p->Srb0.Tx_buffer.payload_size,
...
/*-------gNB Msg.3--------*/
/*
\brief: 1. RRCSetupRequest: RRC_IDLE → The RRC Setup Request is sent with the random ue-Identity and an establishment cause.
        
        2. RRCRRequest: RRC_INACTIVE
        3. RRCReestablishmentRequest: RRC Reestablishment
        4. RRCSystemInfoRequest: other SI request
        5. Handover (competition), through CRNTI + RRCReconfigurationComplete
*/
int rrc_gNB_decode_ccch(
  ...
  const int              CC_id)
{
  ...
  NR_RRCSetupRequest_t           *rrcRequest = NULL;
  NR_RRCReestablishmentRequest_t *rrcReestablishmentRequest = NULL;
  int                             i, rval;
  struct rrc_gNB_ue_context_s    *ue_context_p = NULL;
  uint64_t                        random_value = 0;
  //<5G-GUTI> = <GUAMI><5G-TMSI> 5G globally unique temporary UE identity
  //uint64_t  NR_GUTI_received;
  //<5G-S-TMSI> = <AMF Set ID><AMF Pointer><5G-TMSI> It's a shortened form of 5G-GUTI. When paging, only need to use 5G-S-TMSI.
  ...
  if (ul_ccch_msg->message.present == NR_UL_CCCH_MessageType_PR_c1) {
    switch (ul_ccch_msg->message.choice.c1.present) {
      ...
      case NR_UL_CCCH_MessageType__c1_PR_rrcReestablishmentRequest:
        ...
        LOG_D(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT"MAC_gNB--- MAC_DATA_IND (rrcReestablishmentRequest on SRB0) --> RRC_gNB\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
        rrcReestablishmentRequest =
          &ul_ccch_msg->message.choice.c1.choice.rrcReestablishmentRequest.rrcReestablishmentRequest;
        LOG_I(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT" NR_RRCReestablishmentRequest cause %s\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ((rrcReestablishmentRequest->reestablishmentCause == NR_ReestablishmentCause_otherFailure) ?    "Other Failure" :
               ...
               (rrcReestablishmentRequest->reestablishmentCause == NR_ReestablishmentCause_spare1) ? "Spare1, unknown" :
               "reconfigurationFailure"));
        {//Scope
          uint16_t                          c_rnti = 0;

          if (rrcReestablishmentRequest->ue_Identity.physCellId != RC.rrc[ctxt_pP->module_id]->carrier[CC_id].physCellId) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" NR_rrcReestablishmentRequest ue_Identity.physCellId(%ld) is not equal to current physCellId(%d), shall reject this UE\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
                  rrcReestablishmentRequest->ue_Identity.physCellId,
                  RC.rrc[ctxt_pP->module_id]->carrier[CC_id].physCellId);
            rrc_gNB_generate_RRCReestablishmentReject(ctxt_pP, ue_context_p, CC_id);
            break;
          }
          ...
          for (i = 0; i < rrcReestablishmentRequest->ue_Identity.shortMAC_I.size; i++) {
            LOG_D(RRC, "rrcReestablishmentRequest->ue_Identity.shortMAC_I.buf[%d] = %x\n",
                  i, rrcReestablishmentRequest->ue_Identity.shortMAC_I.buf[i]);
          }

          if (rrcReestablishmentRequest->ue_Identity.c_RNTI.size == 0 ||
              rrcReestablishmentRequest->ue_Identity.c_RNTI.size > 2) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" NR_rrcReestablishmentRequest c_RNTI range error, let's reject the UE\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
            rrc_gNB_generate_RRCReestablishmentReject(ctxt_pP, ue_context_p, CC_id);
            break;
          }

          c_rnti = BIT_STRING_to_uint16(&rrcReestablishmentRequest->ue_Identity.c_RNTI);
          LOG_D(RRC, "c_rnti is %x\n", c_rnti);
          ue_context_p = rrc_gNB_get_ue_context(RC.rrc[ctxt_pP->module_id], c_rnti);

          if (ue_context_p == NULL) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" NR_rrcReestablishmentRequest without UE context, shall reject the UE\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
            rrc_gNB_generate_RRCReestablishmentReject(ctxt_pP, ue_context_p, CC_id);
            break;
          }
          ...
          if(UE_id == -1) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" NR_rrcReestablishmentRequest without UE_id(MAC) rnti %x, shall reject the UE\n",
          ...
          if((RC.mac[ctxt_pP->module_id]->UE_info.UE_sched_ctrl[UE_id].ue_reestablishment_reject_timer > 0) &&
              (RC.mac[ctxt_pP->module_id]->UE_info.UE_sched_ctrl[UE_id].ue_reestablishment_reject_timer_thres > 20)) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" RRCReestablishmentComplete(Previous) don't receive, delete the c-rnti UE\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
            RC.mac[ctxt_pP->module_id]->UE_info.UE_sched_ctrl[UE_id].ue_reestablishment_reject_timer = 1000;
            rrc_gNB_previous_SRB2(ue_context_p);
          ...
          if(previous_rnti != 0) {
            ...
            if(UE_id == -1) {
              LOG_E(RRC,
                    PROTOCOL_RRC_CTXT_UE_FMT" RRCReestablishmentRequest without UE_id(MAC) previous rnti %x, let's reject the UE\n",
            ...
            if((RC.mac[ctxt_pP->module_id]->UE_info.UE_sched_ctrl[UE_id].ue_reestablishment_reject_timer > 0) &&
                (RC.mac[ctxt_pP->module_id]->UE_info.UE_sched_ctrl[UE_id].ue_reestablishment_reject_timer_thres > 20)) {
              LOG_E(RRC,
                    PROTOCOL_RRC_CTXT_UE_FMT" RRCReestablishmentComplete(Previous) don't receive, delete the Previous UE\n",
                    PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
              RC.mac[ctxt_pP->module_id]->UE_info.UE_sched_ctrl[UE_id].ue_reestablishment_reject_timer = 1000;
              rrc_gNB_previous_SRB2(ue_context_p);
              ue_context_p->ue_context.ue_reestablishment_timer = 0;
          ...
          //c-plane not end
          if((ue_context_p->ue_context.Status != RRC_RECONFIGURED) && (ue_context_p->ue_context.reestablishment_cause == NR_ReestablishmentCause_spare1)) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" NR_RRCReestablishmentRequest (UE %x c-plane is not end), let's reject the UE\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),c_rnti);
            rrc_gNB_generate_RRCReestablishmentReject(ctxt_pP, ue_context_p, CC_id);
            break;
          }
          ...
          if(ue_context_p->ue_context.ue_reestablishment_timer > 0) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" RRCReconfigurationComplete(Previous) don't receive, delete the Previous UE\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
          ...
          rrc_pdcp_config_asn1_req(ctxt_pP,
                                   ...
                                   (NR_DRB_ToAddModList_t *) NULL,
                                   (NR_DRB_ToReleaseList_t *) NULL,
                                   0xff,
                                   NULL,
                                   NULL,
                                   NULL
                                   , (NR_PMCH_InfoList_r9_t *) NULL
                                   ,NULL);
      ...
      case NR_UL_CCCH_MessageType__c1_PR_rrcSetupRequest:
        //T(T_GNB_RRC_SETUP_REQUEST, T_INT(ctxt_pP->module_id), T_INT(ctxt_pP->frame),
        //  T_INT(ctxt_pP->subframe), T_INT(ctxt_pP->rnti));
        LOG_DUMPMSG(RRC,DEBUG_RRC,(char *)buffer,
                    buffer_length,
                    "[MSG] RRC Setup Request\n");
        LOG_D(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT"MAC_gNB --- MAC_DATA_IND  (rrcSetupRequest on SRB0) --> RRC_gNB\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
        ue_context_p = rrc_gNB_get_ue_context(
                         RC.rrc[ctxt_pP->module_id],
                         ctxt_pP->rnti);
        ...
        if (ue_context_p != NULL) {
          // erase content  
          // TO DO: /RRC/NR/rrc_gNB_UE_context.c 
          ...
          {//Scope
            if (NR_InitialUE_Identity_PR_randomValue == rrcSetupRequest->ue_Identity.present) {
              //38.331-5.3.3.3 randomvalue is 39-bit in the range 0..2^39-1, i.e. ~5-byte
              ...
              //from buf, copy size bytes to  & random_value + 3 : i.e. use the last 5 bytes
               ...
               */
              if ((ue_context_p = rrc_gNB_ue_context_random_exist(ctxt_pP, random_value))) {
              ...
              ue_context_p = rrc_gNB_get_next_free_ue_context(ctxt_pP, random_value);
              ...
              else if (NR_InitialUE_Identity_PR_s_TMSI == rrcSetupRequest->ue_Identity.present) {
              /* Save s-TMSI */
              //TODO NR_S_TMSI_t? didn't find it in asn1
              NR_S_TMSI_t   s_TMSI = rrcSetupRequest->ue_Identity.choice.s_TMSI;
              mme_code_t mme_code = BIT_STRING_to_uint8(&s_TMSI.mmec);
              m_tmsi_t   m_tmsi   = BIT_STRING_to_uint32(&s_TMSI.m_TMSI);
              random_value = (((uint64_t)mme_code) << 32) | m_tmsi;

              if ((ue_context_p = rrc_gNB_ue_context_stmsi_exist(ctxt_pP, mme_code, m_tmsi))) {
                LOG_I(RRC," S-TMSI exists, ue_context_p %p, old rnti %x => %x\n",ue_context_p,ue_context_p->ue_context.rnti,ctxt_pP->rnti);

                if (!NODE_IS_CU(RC.rrc[ctxt_pP->module_id]->node_type)) {
                  rrc_mac_remove_ue(ctxt_pP->module_id, ue_context_p->ue_context.rnti);
                } else {
                  //TODO F1AP-CU, check intertask_interface.h - we may need more NR FOREACH_TASKs
                ...
                ue_context_p->ue_context.ue_release_timer_rrc = 0;
                ue_context_p->ue_context.reestablishment_xid = -1;
              } //if ue_context_p = rrc_gNB_ue_context_stmsi_exist
                else {
                LOG_I(RRC," S-TMSI doesn't exist, setting Initialue_identity_s_TMSI.m_tmsi to %p => %x\n",ue_context_p,m_tmsi);
                //              ue_context_p = rrc_gNB_get_next_free_ue_context(ctxt_pP, NOT_A_RANDOM_UE_IDENTITY);
                ue_context_p = rrc_gNB_get_next_free_ue_context(ctxt_pP,random_value);
              ...
              }// ue_context_p != rrc_gNB_ue_context_stmsi_exist
              ...
              MSC_LOG_RX_MESSAGE(
                ...
                buffer,
                dec_rval.consumed,
                MSC_AS_TIME_FMT" rrcSetupRequest UE %x size %u (s-TMSI mmec %u m_TMSI %u random UE id (0x%" PRIx64 ")",
                MSC_AS_TIME_ARGS(ctxt_pP),
                ue_context_p->ue_context.rnti,
                dec_rval.consumed,
                ue_context_p->ue_context.Initialue_identity_s_TMSI.mme_code,
              ...
              rrc_gNB_generate_RRCReject(ctxt_pP,
                                                   rrc_gNB_get_ue_context(RC.rrc[ctxt_pP->module_id], ctxt_pP->rnti),
                                                   CC_id);
              break;
            }
          }//Scope
          ...
          if (ue_context_p != NULL) {
            ue_context_p->ue_context.establishment_cause = rrcRequest->establishmentCause;
            ue_context_p->ue_context.reestablishment_cause = NR_ReestablishmentCause_spare1;
        ...
        memcpy(&ue_context_p->ue_context.Srb2.Srb_info.Lchan_desc[1],
               &DCCH_LCHAN_DESC,
               LCHAN_DESC_SIZE);
        rrc_gNB_generate_RRCSetup(ctxt_pP, ue_context_p, CC_id);
        LOG_I(RRC, PROTOCOL_RRC_CTXT_UE_FMT"CALLING RLC CONFIG SRB1 (rbid %d)\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              Idx);
        MSC_LOG_TX_MESSAGE(
        ...
        if (!NODE_IS_CU(RC.rrc[ctxt_pP->module_id]->node_type)) {
          rrc_rlc_config_asn1_req(ctxt_pP,
                                  ...
                                  (NR_DRB_ToAddModList_t *) NULL,
                                  (NR_DRB_ToReleaseList_t *) NULL,
                                  (NR_PMCH_InfoList_r9_t *) NULL, 0, 0
                                 );
        }
...
//-----------------------------------------------------------------------------
{
  static uint8_t                      rrc_transaction_identifier[NUMBER_OF_gNB_MAX];
  rrc_transaction_identifier[gnb_mod_idP] = (rrc_transaction_identifier[gnb_mod_idP] + 1) % RRC_TRANSACTION_IDENTIFIER_NUMBER;
  LOG_T(RRC,"generated xid is %d\n",rrc_transaction_identifier[gnb_mod_idP]);
  return rrc_transaction_identifier[gnb_mod_idP];
}
/*------------------------------------------------------------------------------*/
/* Functions to handle UE index in gNB UE list */


////-----------------------------------------------------------------------------
//static module_id_t
//rrc_gNB_get_UE_index(
//                module_id_t gnb_mod_idP,
...
//    module_id_t    i;
//
//    AssertFatal(gnb_mod_idP < NB_gNB_INST, "gNB index invalid (%d/%d)!", gnb_mod_idP, NB_gNB_INST);
//
//    for (i = 0; i < MAX_MOBILES_PER_ENB; i++) {
//        if (RC.rrc[gnb_mod_idP]->Info.UE_info[i] == UE_identity) {
...
//-----------------------------------------------------------------------------
/*
* Should be called when UE context in gNB should be released
* or when S1 command UE_CONTEXT_RELEASE_REQ should be sent
*/
void
rrc_gNB_free_UE(
  const module_id_t gnb_mod_idP,
  const struct rrc_gNB_ue_context_s *const ue_context_pP)
//-----------------------------------------------------------------------------
{
  rnti_t rnti = ue_context_pP->ue_context.rnti;

  if (gnb_mod_idP >= NB_gNB_INST) {
    LOG_E(RRC, "gNB instance invalid (%d/%d) for UE %x!\n",
          gnb_mod_idP,
          NB_gNB_INST,
          rnti);
    return;
  ...
  if(EPC_MODE_ENABLED) {
    if (!NODE_IS_DU(RC.rrc[gnb_mod_idP]->node_type)) {
      if((ue_context_pP->ue_context.ul_failure_timer >= 20000) && (mac_gNB_get_rrc_status(gnb_mod_idP, rnti) >= RRC_CONNECTED)) {
        LOG_I(RRC, "[gNB %d] S1AP_UE_CONTEXT_RELEASE_REQ sent for RNTI %x, cause 21, radio connection with ue lost\n",
              gnb_mod_idP,
              rnti);
        rrc_gNB_send_S1AP_UE_CONTEXT_RELEASE_REQ(gnb_mod_idP,
            ue_context_pP,
            S1AP_CAUSE_RADIO_NETWORK,
            21); // send cause 21: radio connection with ue lost
        /* From 3GPP 36300v10 p129 : 19.2.2.2.2 S1 UE Context Release Request (gNB triggered)
         * If the E-UTRAN internal reason is a radio link failure detected in the gNB, the gNB shall wait a sufficient time before
      ...
      if((ue_context_pP->ue_context.ue_rrc_inactivity_timer >= RC.rrc[gnb_mod_idP]->configuration.rrc_inactivity_timer_thres) &&
          (mac_gNB_get_rrc_status(gnb_mod_idP, rnti) >= RRC_CONNECTED) &&
          (RC.rrc[gnb_mod_idP]->configuration.rrc_inactivity_timer_thres > 0)) {
        LOG_I(RRC, "[gNB %d] S1AP_UE_CONTEXT_RELEASE_REQ sent for RNTI %x, cause 20, user inactivity\n",
              gnb_mod_idP,
              rnti);
        rrc_gNB_send_S1AP_UE_CONTEXT_RELEASE_REQ(gnb_mod_idP,
            ue_context_pP,
  ...
  LOG_W(RRC, "[gNB %d] Removing UE RNTI %x\n",
        gnb_mod_idP,
        rnti);
  // add UE info to freeList
  LOG_I(RRC, "Put UE %x into freeList\n",
        rnti);
  put_UE_in_freelist(gnb_mod_idP, rnti, 1);
}

void remove_UE_from_freelist(module_id_t mod_id, rnti_t rnti) {
  gNB_MAC_INST                             *gNB_MAC = RC.mac[mod_id];
  pthread_mutex_lock(&lock_ue_freelist);
  UE_free_list_t                           *free_list = &gNB_MAC->UE_free_list;
...
void put_UE_in_freelist(module_id_t mod_id, rnti_t rnti, boolean_t removeFlag) {
  ...
  gNB_MAC_INST                             *gNB_MAC = RC.mac[mod_id];
  pthread_mutex_lock(&lock_ue_freelist);
  free_list = &gNB_MAC->UE_free_list;
  ...
  free_list->num_UEs++;
  gNB_MAC->UE_release_req.ue_release_request_body.ue_release_request_TLVs_list[gNB_MAC->UE_release_req.ue_release_request_body.number_of_TLVs].rnti = rnti;
  gNB_MAC->UE_release_req.ue_release_request_body.number_of_TLVs++;
  free_list->tail_freelist = (free_list->tail_freelist + 1) % (NUMBER_OF_UE_MAX+1);
  pthread_mutex_unlock(&lock_ue_freelist);
}

extern int16_t find_dlsch(uint16_t rnti, PHY_VARS_gNB *gNB,find_type_t type);
extern int16_t find_ulsch(uint16_t rnti, PHY_VARS_gNB *gNB,find_type_t type);
extern void clean_eNb_ulsch(NR_gNB_ULSCH_t *ulsch);
extern void clean_eNb_dlsch(NR_gNB_DLSCH_t *dlsch);

void release_UE_in_freeList(module_id_t mod_id) {
  int i, j, CC_id, pdu_number;
  protocol_ctxt_t                           ctxt;
  nfapi_ul_config_request_body_t           *ul_req_tmp = NULL;
  PHY_VARS_gNB                             *gNB_PHY = NULL;
  struct rrc_gNB_ue_context_s              *ue_context_pP = NULL;
  gNB_MAC_INST                             *gNB_MAC = RC.mac[mod_id];
  boolean_t                                 remove_UEContext;
  rnti_t                                    rnti;
  int                                       head, tail, ue_num;
  pthread_mutex_lock(&lock_ue_freelist);
  head = gNB_MAC->UE_free_list.head_freelist;
  tail = gNB_MAC->UE_free_list.tail_freelist;
  ...
  if(tail < head) {
    tail = head + gNB_MAC->UE_free_list.num_UEs;
  }
  ...
  for(ue_num = head; ue_num < tail; ue_num++) {
    ...
    rnti = gNB_MAC->UE_free_list.UE_free_ctrl[ue_num].rnti;

    if(rnti != 0) {
      remove_UEContext = gNB_MAC->UE_free_list.UE_free_ctrl[ue_num].removeContextFlg;
      PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, mod_id, ENB_FLAG_YES, rnti, 0, 0,mod_id);

      for (CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
        gNB_PHY = RC.gNB[mod_id][CC_id];

	int id;

	// clean ULSCH entries for rnti
	id = find_ulsch(rnti,gNB_PHY,SEARCH_EXIST);
        if (id>=0) clean_eNb_ulsch(gNB_PHY->ulsch[id]);

	// clean DLSCH entries for rnti
	id = find_dlsch(rnti,gNB_PHY,SEARCH_EXIST);
        if (id>=0) clean_eNb_dlsch(gNB_PHY->dlsch[id][0]);

	// clean UCI entries for rnti
        for (i=0; i<NUMBER_OF_UCI_VARS_MAX; i++) {
          if(gNB_PHY->uci_vars[i].rnti == rnti) {
            LOG_I(MAC, "clean eNb uci_vars[%d] UE %x \n",i, rnti);
            memset(&gNB_PHY->uci_vars[i],0,sizeof(NR_gNB_UCI));
          }
        }
		
	/*
        for (i=0; i<MAX_MOBILES_PER_ENB; i++) {
          ulsch = gNB_PHY->ulsch[i];

          if((ulsch != NULL) && (ulsch->rnti == rnti)) {
            void clean_eNb_ulsch(NR_gNB_ULSCH_t *ulsch);
            LOG_I(RRC, "clean_eNb_ulsch ulsch[%d] UE %x\n", i, rnti);
            clean_eNb_ulsch(ulsch);
          }

          dlsch = gNB_PHY->dlsch[i][0];

          if((dlsch != NULL) && (dlsch->rnti == rnti)) {
            void clean_eNb_dlsch(NR_gNB_DLSCH_t *dlsch);
	...
	*/
        ...
        for(j = 0; j < 10; j++) {
          ul_req_tmp = &gNB_MAC->UL_req_tmp[CC_id][j].ul_config_request_body;
      ...
      if(remove_UEContext) {
        ue_context_pP = rrc_gNB_get_ue_context(RC.rrc[mod_id],rnti);

        if(ue_context_pP) {
          rrc_gNB_remove_ue_context(&ctxt,RC.rrc[mod_id],
...
rrc_gNB_process_RRCSetupComplete(
  const protocol_ctxt_t *const ctxt_pP,
  rrc_gNB_ue_context_t *ue_context_pP,
  NR_RRCSetupComplete_r8_IEs_t *rrcSetupComplete
)
//-----------------------------------------------------------------------------
{
  LOG_I(RRC, PROTOCOL_RRC_CTXT_UE_FMT" [RAPROC] Logical Channel UL-DCCH, " "processing NR_RRCSetupComplete from UE (SRB1 Active)\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
  ...
  if (EPC_MODE_ENABLED == 1) {
    ...
    rrc_gNB_send_S1AP_NAS_FIRST_REQ(ctxt_pP, ue_context_pP, rrcSetupComplete);
  } else {
...
//-----------------------------------------------------------------------------
void
rrc_gNB_generate_NR_UECapabilityEnquiry(
  const protocol_ctxt_t *const ctxt_pP,
  rrc_gNB_ue_context_t          *const ue_context_pP
)
//-----------------------------------------------------------------------------
...
rrc_gNB_process_RRCReestablishmentComplete(
  ...
  rrc_gNB_ue_context_t         *ue_context_pP,
  const uint8_t xid,
  NR_RRCReestablishmentComplete_r8_IEs_t *NR_RRCReestablishmentComplete
)
//-----------------------------------------------------------------------------
{
  LOG_I(RRC,
  ...
  struct NR_SRB_ToAddMod                *SRB2_config = NULL;
  struct NR_DRB_ToAddMod                *DRB_config = NULL;
  int i = 0;
  uint8_t                             buffer[RRC_BUF_SIZE];
  uint16_t                            size;
  NR_MeasObjectToAddModList_t       *MeasObj_list                     = NULL;
  NR_MeasObjectToAddMod_t           *MeasObj                          = NULL;
  NR_ReportConfigToAddModList_t     *ReportConfig_list                = NULL;
  NR_ReportConfigToAddMod_t         *ReportConfig_per, *ReportConfig_A1,
                                     *ReportConfig_A2, *ReportConfig_A3, *ReportConfig_A4, *ReportConfig_A5;
  NR_MeasIdToAddModList_t           *MeasId_list                      = NULL;
  NR_MeasIdToAddMod_t               *MeasId0, *MeasId1, *MeasId2, *MeasId3, *MeasId4, *MeasId5;
  NR_RSRP_Range_t                   *rsrp                             = NULL;
  struct NR_MeasConfig__speedStatePars  *Sparams                          = NULL;
  NR_QuantityConfig_t                   *quantityConfig                   = NULL;
  NR_CellsToAddMod_t                    *CellToAdd                        = NULL;
  NR_CellsToAddModList_t                *CellsToAddModList                = NULL;
  struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *dedicatedInfoNASList = NULL;
  NR_DedicatedInfoNAS_t                 *dedicatedInfoNas                 = NULL;
  /* for no gcc warnings */
  (void)dedicatedInfoNas;
  NR_C_RNTI_t                           *cba_RNTI                         = NULL;
  ...
  *DRB_configList2 = CALLOC(1, sizeof(**DRB_configList2));
  ...
  if (DRB_configList != NULL) {
    ...
    for (i = 0; (i < DRB_configList->list.count) && (i < 3); i++) {
      ...
      // Add DRB to DRB configuration list, for NR_RRCReconfigurationComplete
  ...
  if (EPC_MODE_ENABLED) {
    ...
    uint32_t gNB_ue_s1ap_id = ue_context_pP->ue_context.gNB_ue_s1ap_id;
    gNB_RRC_INST *rrc_instance_p = RC.rrc[ENB_INSTANCE_TO_MODULE_ID(ctxt_pP->instance)];

    if (gNB_ue_s1ap_id > 0) {
      h_rc = hashtable_get(rrc_instance_p->s1ap_id2_s1ap_ids, (hash_key_t)gNB_ue_s1ap_id, (void **)&rrc_ue_s1ap_ids_p);
    ...
    if ( ret != 0 ) {
      ...
      ue_context_pP->ue_context.ul_failure_timer = 20000; // set ul_failure to 20000 for triggering rrc_gNB_send_S1AP_UE_CONTEXT_RELEASE_REQ
      rrc_gNB_free_UE(ctxt_pP->module_id,ue_context_pP);
  ...
  //  NR_RRCReconfiguration->criticalExtensions.choice.c1.choice.rrcReconfiguration_r8.measConfig->measIdToAddModList = MeasId_list;
  ...
  MeasObj->measObject.present = NR_MeasObjectToAddMod__measObject_PR_measObjectEUTRA;
  MeasObj->measObject.choice.measObjectEUTRA.carrierFreq = 3350; //band 7, 2.68GHz
  //MeasObj->measObject.choice.measObjectEUTRA.carrierFreq = 36090; //band 33, 1.909GHz
  MeasObj->measObject.choice.measObjectEUTRA.allowedMeasBandwidth = NR_AllowedMeasBandwidth_mbw25;
  ...
  MeasObj->measObject.choice.measObjectEUTRA.cellsToAddModList =
    (NR_CellsToAddModList_t *) CALLOC(1, sizeof(*CellsToAddModList));
  CellsToAddModList = MeasObj->measObject.choice.measObjectEUTRA.cellsToAddModList;

  // Add adjacent cell lists (6 per gNB)
  for (i = 0; i < 6; i++) {
    CellToAdd = (NR_CellsToAddMod_t *) CALLOC(1, sizeof(*CellToAdd));
    CellToAdd->cellIndex = i + 1;
    CellToAdd->physCellId = get_adjacent_cell_id(ctxt_pP->module_id, i);
    CellToAdd->cellIndividualOffset = NR_Q_OffsetRange_dB0;
    ASN_SEQUENCE_ADD(&CellsToAddModList->list, CellToAdd);
  }

  ASN_SEQUENCE_ADD(&MeasObj_list->list, MeasObj);
  //  NR_RRCReconfiguration->criticalExtensions.choice.c1.choice.rrcReconfiguration_r8.measConfig->measObjectToAddModList = MeasObj_list;
  ...
  ReportConfig_per->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.triggerType.present =
    NR_ReportConfigEUTRA__triggerType_PR_periodical;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.triggerType.choice.periodical.purpose =
    NR_ReportConfigEUTRA__triggerType__periodical__purpose_reportStrongestCells;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.triggerQuantity = NR_ReportConfigEUTRA__triggerQuantity_rsrp;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.reportQuantity = NR_ReportConfigEUTRA__reportQuantity_both;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.maxReportCells = 2;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.reportInterval = NR_ReportInterval_ms120;
  ReportConfig_per->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
  ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_per);
  ReportConfig_A1->reportConfigId = 2;
  ReportConfig_A1->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.triggerType.present =
    NR_ReportConfigEUTRA__triggerType_PR_event;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.present =
    NR_ReportConfigEUTRA__triggerType__event__eventId_PR_eventA1;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.eventA1.
  a1_Threshold.present = NR_ThresholdEUTRA_PR_threshold_RSRP;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.eventA1.
  a1_Threshold.choice.threshold_RSRP = 10;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.triggerQuantity = NR_ReportConfigEUTRA__triggerQuantity_rsrp;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.reportQuantity = NR_ReportConfigEUTRA__reportQuantity_both;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.maxReportCells = 2;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.reportInterval = NR_ReportInterval_ms120;
  ReportConfig_A1->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
  ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_A1);

  if (RC.rrc[ctxt_pP->module_id]->HO_flag == 1 /*HO_MEASUREMENT */ ) {
    LOG_I(RRC, "[gNB %d] frame %d: requesting A2, A3, A4, A5, and A6 event reporting\n",
          ctxt_pP->module_id, ctxt_pP->frame);
    ReportConfig_A2->reportConfigId = 3;
    ReportConfig_A2->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.triggerType.present =
      NR_ReportConfigEUTRA__triggerType_PR_event;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.present =
      NR_ReportConfigEUTRA__triggerType__event__eventId_PR_eventA2;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA2.a2_Threshold.present = NR_ThresholdEUTRA_PR_threshold_RSRP;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA2.a2_Threshold.choice.threshold_RSRP = 10;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.triggerQuantity =
      NR_ReportConfigEUTRA__triggerQuantity_rsrp;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.reportQuantity = NR_ReportConfigEUTRA__reportQuantity_both;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.maxReportCells = 2;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.reportInterval = NR_ReportInterval_ms120;
    ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
    ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_A2);
    ReportConfig_A3->reportConfigId = 4;
    ReportConfig_A3->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.triggerType.present =
      NR_ReportConfigEUTRA__triggerType_PR_event;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.present =
      NR_ReportConfigEUTRA__triggerType__event__eventId_PR_eventA3;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.eventA3.a3_Offset = 1;   //10;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA3.reportOnLeave = 1;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.triggerQuantity =
      NR_ReportConfigEUTRA__triggerQuantity_rsrp;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.reportQuantity = NR_ReportConfigEUTRA__reportQuantity_both;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.maxReportCells = 2;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.reportInterval = NR_ReportInterval_ms120;
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
    ...
    ReportConfig_A3->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.timeToTrigger =
      NR_TimeToTrigger_ms40;
    ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_A3);
    ReportConfig_A4->reportConfigId = 5;
    ReportConfig_A4->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.triggerType.present =
      NR_ReportConfigEUTRA__triggerType_PR_event;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.present =
      NR_ReportConfigEUTRA__triggerType__event__eventId_PR_eventA4;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA4.a4_Threshold.present = NR_ThresholdEUTRA_PR_threshold_RSRP;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA4.a4_Threshold.choice.threshold_RSRP = 10;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.triggerQuantity =
      NR_ReportConfigEUTRA__triggerQuantity_rsrp;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.reportQuantity = NR_ReportConfigEUTRA__reportQuantity_both;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.maxReportCells = 2;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.reportInterval = NR_ReportInterval_ms120;
    ReportConfig_A4->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
    ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_A4);
    ReportConfig_A5->reportConfigId = 6;
    ReportConfig_A5->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.triggerType.present =
      NR_ReportConfigEUTRA__triggerType_PR_event;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.present =
      NR_ReportConfigEUTRA__triggerType__event__eventId_PR_eventA5;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA5.a5_Threshold1.present = NR_ThresholdEUTRA_PR_threshold_RSRP;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    eventA5.a5_Threshold2.present = NR_ThresholdEUTRA_PR_threshold_RSRP;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.triggerType.choice.event.eventId.choice.
    ...
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.triggerQuantity =
      NR_ReportConfigEUTRA__triggerQuantity_rsrp;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.reportQuantity = NR_ReportConfigEUTRA__reportQuantity_both;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.maxReportCells = 2;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.reportInterval = NR_ReportInterval_ms120;
    ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
    ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_A5);
    //  NR_RRCReconfiguration->criticalExtensions.choice.c1.choice.rrcReconfiguration_r8.measConfig->reportConfigToAddModList = ReportConfig_list;
#if 0
     ...
     */
    rsrp = CALLOC(1, sizeof(NR_RSRP_Range_t));
    *rsrp = 20;
#endif
    Sparams = CALLOC(1, sizeof(*Sparams));
    Sparams->present = NR_MeasConfig__speedStatePars_PR_setup;
    Sparams->choice.setup.timeToTrigger_SF.sf_High = NR_SpeedStateScaleFactors__sf_Medium_oDot75;
    Sparams->choice.setup.timeToTrigger_SF.sf_Medium = NR_SpeedStateScaleFactors__sf_High_oDot5;
    Sparams->choice.setup.mobilityStateParameters.n_CellChangeHigh = 10;
    Sparams->choice.setup.mobilityStateParameters.n_CellChangeMedium = 5;
    Sparams->choice.setup.mobilityStateParameters.t_Evaluation = NR_MobilityStateParameters__t_Evaluation_s60;
    Sparams->choice.setup.mobilityStateParameters.t_HystNormal = NR_MobilityStateParameters__t_HystNormal_s120;
    quantityConfig = CALLOC(1, sizeof(*quantityConfig));
    memset((void *)quantityConfig, 0, sizeof(*quantityConfig));
    quantityConfig->quantityConfigEUTRA = CALLOC(1, sizeof(struct NR_QuantityConfigEUTRA));
    ...
    *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRP = NR_FilterCoefficient_fc4;
    *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRQ = NR_FilterCoefficient_fc4;
    LOG_I(RRC,
          "[gNB %d] Frame %d: potential handover preparation: store the information in an intermediate structure in case of failure\n",
          ctxt_pP->module_id, ctxt_pP->frame);
    // store the information in an intermediate structure for Hanodver management
    //rrc_inst->handover_info.as_config.sourceRadioResourceConfig.srb_ToAddModList = CALLOC(1,sizeof());
    ue_context_pP->ue_context.handover_info = CALLOC(1, sizeof(*(ue_context_pP->ue_context.handover_info)));
    //memcpy((void *)rrc_inst->handover_info[ue_mod_idP]->as_config.sourceRadioResourceConfig.srb_ToAddModList,(void *)SRB_list,sizeof(NR_SRB_ToAddModList_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.srb_ToAddModList = *SRB_configList2;
    //memcpy((void *)rrc_inst->handover_info[ue_mod_idP]->as_config.sourceRadioResourceConfig.drb_ToAddModList,(void *)DRB_list,sizeof(NR_DRB_ToAddModList_t));
    ...
    memcpy((void *)ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.mac_MainConfig,
           (void *)ue_context_pP->ue_context.mac_MainConfig, sizeof(NR_MAC_MainConfig_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.physicalConfigDedicated =
      CALLOC(1, sizeof(NR_PhysicalConfigDedicated_t));
    memcpy((void *)ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.physicalConfigDedicated,
           (void *)ue_context_pP->ue_context.physicalConfigDedicated, sizeof(NR_PhysicalConfigDedicated_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.sps_Config = NULL;
    //memcpy((void *)rrc_inst->handover_info[ue_mod_idP]->as_config.sourceRadioResourceConfig.sps_Config,(void *)rrc_inst->sps_Config[ue_mod_idP],sizeof(SPS_Config_t));
  }

  /* Initialize NAS list */
  dedicatedInfoNASList = CALLOC(1, sizeof(struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList));

  /* Add all NAS PDUs to the list */
  for (i = 0; i < ue_context_pP->ue_context.nb_of_e_rabs; i++) {
    if (ue_context_pP->ue_context.e_rab[i].param.nas_pdu.buffer != NULL) {
      dedicatedInfoNas = CALLOC(1, sizeof(NR_DedicatedInfoNAS_t));
      ...
      LOG_D(RRC, "Add NR_DedicatedInfoNAS(%d) to NR_DedicatedInfoNASList\n", i);
  ...
  // send NR_RRCReconfiguration
  memset(buffer, 0, RRC_BUF_SIZE);
  size = do_RRCReconfiguration(ctxt_pP,
                                         buffer,
                                         next_xid,   //Transaction_id,
                                         (NR_SRB_ToAddModList_t *)*SRB_configList2, // SRB_configList
                                         (NR_DRB_ToAddModList_t *)DRB_configList,
                                         (NR_DRB_ToReleaseList_t *)NULL, // DRB2_list,
                                         (struct NR_SPS_Config *)NULL,   // maybe ue_context_pP->ue_context.sps_Config,
                                         (struct NR_PhysicalConfigDedicated *)ue_context_pP->ue_context.physicalConfigDedicated,
                                         //#ifdef EXMIMO_IOT
                                         //                                         NULL, NULL, NULL,NULL,
                                         //#else
                                         measurements_enabled ? (NR_MeasObjectToAddModList_t *)MeasObj_list : NULL, // MeasObj_list,
                                         measurements_enabled ? (NR_ReportConfigToAddModList_t *)ReportConfig_list : NULL, // ReportConfig_list,
                                         measurements_enabled ? (NR_QuantityConfig_t *)quantityConfig : NULL, //quantityConfig,
                                         (NR_MeasIdToAddModList_t *)NULL,
                                         //#endif
                                         (NR_MAC_MainConfig_t *)ue_context_pP->ue_context.mac_MainConfig,
                                         (NR_MeasGapConfig_t *)NULL,
                                         (NR_MobilityControlInfo_t *)NULL,
                                         (NR_SecurityConfigHO_t *)NULL,
                                         (struct NR_MeasConfig__speedStatePars *)Sparams, // Sparams,
                                         (NR_RSRP_Range_t *)rsrp, // rsrp,
                                         (NR_C_RNTI_t *)cba_RNTI, // cba_RNTI
                                         (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *)dedicatedInfoNASList, //dedicatedInfoNASList
                                         (NR_SL_CommConfig_r12_t *)NULL,
                                         (NR_SL_DiscConfig_r12_t *)NULL,
                                         (NR_SCellToAddMod_r10_t *)NULL
                                        );
  ...
  if(size==65535) {
    LOG_E(RRC,"RRC decode err!!! do_RRCReconfiguration\n");
  ...
  //  nfapi_ul_config_request_body_t *ul_req_tmp = NULL;
  //  PHY_VARS_gNB *gNB_PHY = NULL;
  //  gNB_MAC_INST *gNB_MAC = RC.mac[ctxt_prior.module_id];
  //  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
  //    gNB_PHY = RC.gNB[ctxt_prior.module_id][CC_id];
  ...
  LOG_I(RRC, "[RRCReestablishment]put UE %x into freeList\n", reestablish_rnti);
  put_UE_in_freelist(ctxt_pP->module_id, reestablish_rnti, 0);
}
...
//-----------------------------------------------------------------------------
{
  ...
  // X2AP_ENDC_SGNB_RELEASE_REQ(msg).MgNB_ue_x2_id = ;
  ...
  LOG_I(RRC,
        "[gNB %d] frame %d UE rnti %x transmitting sgnb release request to sgnb \n",
...
//-----------------------------------------------------------------------------
{
  ...
  if(ue_context_pP != NULL) {
    if(ue_context_pP->ue_context.Status == RRC_NR_NSA) {
      //rrc_gNB_generate_SgNBReleaseRequest(ctxt_pP,ue_context_pP);
    }
  }

#endif
  size = do_RRCRelease(ctxt_pP->module_id, buffer,rrc_gNB_get_next_transaction_identifier(ctxt_pP->module_id));
  ue_context_pP->ue_context.ue_reestablishment_timer = 0;
  ue_context_pP->ue_context.ue_release_timer = 0;
  ue_context_pP->ue_context.ue_rrc_inactivity_timer = 0;
  LOG_I(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT" Logical Channel DL-DCCH, Generate RRCRelease (bytes %d)\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size);
  LOG_D(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT" --- PDCP_DATA_REQ/%d Bytes (rrcRelease MUI %d) --->[PDCP][RB %u]\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size,
        rrc_gNB_mui,
        DCCH);
  ...
  for (release_num = 0; release_num < NUMBER_OF_UE_MAX; release_num++) {
    if (rrc_release_info.RRC_release_ctrl[release_num].flag == 0) {
      ...
      rrc_release_info.RRC_release_ctrl[release_num].rrc_gNB_mui = rrc_gNB_mui;
...
//-----------------------------------------------------------------------------
{
  ...
  struct NR_PDCP_Config__rlc_AM         *PDCP_rlc_AM                      = NULL;
  ...
  uint8_t xid = rrc_gNB_get_next_transaction_identifier(ctxt_pP->module_id);   //Transaction_id,
  ...
  *DRB_configList2 = CALLOC(1, sizeof(**DRB_configList2));
  ...
  for ( i = 0  ;
    ...
    *(DRB_config->eps_BearerIdentity) = ue_context_pP->ue_context.e_rab[i].param.e_rab_id;//+ 4; // especial case generation
    //   DRB_config->drb_Identity =  1 + drb_identity_index + e_rab_done;// + i ;// (NR_DRB_Identity_t) ue_context_pP->ue_context.e_rab[i].param.e_rab_id;
    // 1 + drb_identiy_index;
    ...
    *DRB_pdcp_config->discardTimer = NR_PDCP_Config__discardTimer_infinity;
    ...
    switch (ue_context_pP->ue_context.e_rab[i].param.qos.qci) {
      ...
      case 66: // 100ms, 10^-2, p2, non-mission critical  voice , GBR
        // RLC
        DRB_rlc_config->present = NR_RLC_Config_PR_um_Bi_Directional;
        DRB_rlc_config->choice.um_Bi_Directional.ul_UM_RLC.sn_FieldLength = NR_SN_FieldLength_size10;
        DRB_rlc_config->choice.um_Bi_Directional.dl_UM_RLC.sn_FieldLength = NR_SN_FieldLength_size10;
        DRB_rlc_config->choice.um_Bi_Directional.dl_UM_RLC.t_Reordering = NR_T_Reordering_ms35;
        // PDCP
        PDCP_rlc_UM = CALLOC(1, sizeof(*PDCP_rlc_UM));
        DRB_pdcp_config->rlc_UM = PDCP_rlc_UM;
        PDCP_rlc_UM->pdcp_SN_Size = NR_PDCP_Config__rlc_UM__pdcp_SN_Size_len12bits;
        break;
      ...
      case 70: // 200ms, 10^-6, p5.5, mision critical data
        // RLC
        DRB_rlc_config->present = NR_RLC_Config_PR_am;
        DRB_rlc_config->choice.am.ul_AM_RLC.t_PollRetransmit = NR_T_PollRetransmit_ms50;
        DRB_rlc_config->choice.am.ul_AM_RLC.pollPDU = NR_PollPDU_p16;
        DRB_rlc_config->choice.am.ul_AM_RLC.pollByte = NR_PollByte_kBinfinity;
        DRB_rlc_config->choice.am.ul_AM_RLC.maxRetxThreshold = NR_UL_AM_RLC__maxRetxThreshold_t8;
        DRB_rlc_config->choice.am.dl_AM_RLC.t_Reordering = NR_T_Reordering_ms35;
        DRB_rlc_config->choice.am.dl_AM_RLC.t_StatusProhibit = NR_T_StatusProhibit_ms25;
        // PDCP
    ...
    DRB_pdcp_config->headerCompression.present = NR_PDCP_Config__headerCompression_PR_notUsed;
    ...
    DRB_ul_SpecificParameters->prioritisedBitRate = NR_LogicalChannelConfig__ul_SpecificParameters__prioritisedBitRate_kBps8;
  ...
  size = do_RRCReconfiguration(ctxt_pP,
                                         ...
                                         (NR_DRB_ToAddModList_t *)*DRB_configList2,
                                         (NR_DRB_ToReleaseList_t *)NULL, // DRB2_list,
                                         (struct NR_SPS_Config *)NULL,   // *sps_Config,
                                         NULL, NULL, NULL, NULL,NULL,
                                         NULL, NULL,  NULL, NULL, NULL, NULL, NULL,
                                         (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *)dedicatedInfoNASList,
                                         (NR_SL_CommConfig_r12_t *)NULL,
                                         (NR_SL_DiscConfig_r12_t *)NULL,
                                         (NR_SCellToAddMod_r10_t *)NULL
                                        );
...
int
rrc_gNB_modify_dedicatedRRCReconfiguration(const protocol_ctxt_t *const ctxt_pP,
    rrc_gNB_ue_context_t          *const ue_context_pP,
    const uint8_t                ho_state
                                                    )
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void
rrc_gNB_generate_dedicatedRRCReconfiguration_release(  const protocol_ctxt_t   *const ctxt_pP,
    rrc_gNB_ue_context_t    *const ue_context_pP,
    uint8_t                  xid,
    uint32_t                 nas_length,
    uint8_t                 *nas_buffer)
//-----------------------------------------------------------------------------
{
  ...
  *DRB_Release_configList2 = CALLOC(1, sizeof(**DRB_Release_configList2));
  ...
  size = do_RRCReconfiguration(ctxt_pP,
                                         ...
                                         NULL,
                                         NULL,
                                         (NR_DRB_ToReleaseList_t *)*DRB_Release_configList2,
                                         NULL,
                                         NULL,
                                         NULL,
                                         NULL,
 ...
 */
void rrc_gNB_generate_defaultRRCReconfiguration(const protocol_ctxt_t *const ctxt_pP,
    rrc_gNB_ue_context_t  *const ue_context_pP,
    const uint8_t                ho_state)
//-----------------------------------------------------------------------------
{
  ...
  /* Configure SRB1/SRB2, PhysicalConfigDedicated, NR_MAC_MainConfig for UE */
  gNB_RRC_INST                           *rrc_inst = RC.rrc[ctxt_pP->module_id];
  struct NR_PhysicalConfigDedicated    **physicalConfigDedicated = &ue_context_pP->ue_context.physicalConfigDedicated;
  struct NR_SRB_ToAddMod                *SRB2_config                      = NULL;
  struct NR_SRB_ToAddMod__rlc_Config    *SRB2_rlc_config                  = NULL;
  struct NR_SRB_ToAddMod__logicalChannelConfig *SRB2_lchan_config         = NULL;
  struct NR_LogicalChannelConfig__ul_SpecificParameters
  ...
  NR_DRB_ToAddModList_t                **DRB_configList = &ue_context_pP->ue_context.DRB_configList;
  NR_DRB_ToAddModList_t                **DRB_configList2                  = NULL;
  NR_MAC_MainConfig_t                   *mac_MainConfig                   = NULL;
  NR_MeasObjectToAddModList_t           *MeasObj_list                     = NULL;
  NR_MeasObjectToAddMod_t               *MeasObj                          = NULL;
  NR_MeasObjectToAddMod_t               *MeasObj2                         = NULL;
  NR_ReportConfigToAddModList_t         *ReportConfig_list                = NULL;
  NR_ReportConfigToAddMod_t             *ReportConfig_per, *ReportConfig_A1,
                                         *ReportConfig_A2, *ReportConfig_A3, *ReportConfig_A4, *ReportConfig_A5;
  NR_ReportConfigToAddMod_t             *ReportConfig_NR                  = NULL;
  NR_MeasIdToAddModList_t               *MeasId_list                      = NULL;
  NR_MeasIdToAddMod_t                   *MeasId0, *MeasId1, *MeasId2, *MeasId3, *MeasId4, *MeasId5;
  NR_MeasIdToAddMod_t                   *MeasId6;
  ...
  NR_UE_EUTRA_Capability_t *UEcap = ue_context_pP->ue_context.UE_Capability;
  ...
  *SRB_configList2 = CALLOC(1, sizeof(**SRB_configList2));
  ...
  SRB2_rlc_config->present = NR_SRB_ToAddMod__rlc_Config_PR_explicitValue;
  SRB2_rlc_config->choice.explicitValue.present = NR_RLC_Config_PR_am;
  SRB2_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.t_PollRetransmit = NR_T_PollRetransmit_ms15;
  SRB2_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.pollPDU = NR_PollPDU_p8;
  SRB2_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.pollByte = NR_PollByte_kB1000;
  SRB2_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.maxRetxThreshold = NR_UL_AM_RLC__maxRetxThreshold_t32;
  SRB2_rlc_config->choice.explicitValue.choice.am.dl_AM_RLC.t_Reordering = NR_T_Reordering_ms35;
  SRB2_rlc_config->choice.explicitValue.choice.am.dl_AM_RLC.t_StatusProhibit = NR_T_StatusProhibit_ms10;
  SRB2_lchan_config = CALLOC(1, sizeof(*SRB2_lchan_config));
  SRB2_config->logicalChannelConfig = SRB2_lchan_config;
  SRB2_lchan_config->present = NR_SRB_ToAddMod__logicalChannelConfig_PR_explicitValue;
  SRB2_ul_SpecificParameters = CALLOC(1, sizeof(*SRB2_ul_SpecificParameters));
  SRB2_ul_SpecificParameters->priority = 3; // let some priority for SRB1 and dedicated DRBs
  SRB2_ul_SpecificParameters->prioritisedBitRate = NR_LogicalChannelConfig__ul_SpecificParameters__prioritisedBitRate_infinity;
  SRB2_ul_SpecificParameters->bucketSizeDuration = NR_LogicalChannelConfig__ul_SpecificParameters__bucketSizeDuration_ms50;
  ...
  *(DRB_config->eps_BearerIdentity) = 5L; // LW set to first value, allowed value 5..15, value : x+4
  ...
  DRB_config->drb_Identity = (NR_DRB_Identity_t) 1;  // (ue_mod_idP+1); //allowed values 1..32, value: x
  DRB_config->logicalChannelIdentity = CALLOC(1, sizeof(long));
  ...
  *DRB_pdcp_config->discardTimer = NR_PDCP_Config__discardTimer_infinity;
  ...
  DRB_ul_SpecificParameters->prioritisedBitRate = NR_LogicalChannelConfig__ul_SpecificParameters__prioritisedBitRate_kBps8; // LogicalChannelConfig__ul_SpecificParameters__prioritisedBitRate_infinity;
  DRB_ul_SpecificParameters->bucketSizeDuration = NR_LogicalChannelConfig__ul_SpecificParameters__bucketSizeDuration_ms50;
  ...
  *periodicBSR_Timer = NR_PeriodicBSR_Timer_r12_sf64; //NR_PeriodicBSR_Timer_r12_infinity; // NR_PeriodicBSR_Timer_r12_sf64; // NR_PeriodicBSR_Timer_r12_sf20
  mac_MainConfig->ul_SCH_Config->periodicBSR_Timer = periodicBSR_Timer;
  mac_MainConfig->ul_SCH_Config->retxBSR_Timer = NR_RetxBSR_Timer_r12_sf320; // NR_RetxBSR_Timer_r12_sf320; // NR_RetxBSR_Timer_r12_sf5120
  mac_MainConfig->ul_SCH_Config->ttiBundling = 0; // FALSE
  mac_MainConfig->timeAlignmentTimerDedicated = NR_TimeAlignmentTimer_infinity;
  /* PHR reconfiguration */
  mac_MainConfig->phr_Config = CALLOC(1, sizeof(*mac_MainConfig->phr_Config));
  mac_MainConfig->phr_Config->present = NR_MAC_MainConfig__phr_Config_PR_setup;
  mac_MainConfig->phr_Config->choice.setup.periodicPHR_Timer =
    NR_MAC_MainConfig__phr_Config__setup__periodicPHR_Timer_sf500; // sf20 = 20 subframes // NR_MAC_MainConfig__phr_Config__setup__periodicPHR_Timer_infinity
  mac_MainConfig->phr_Config->choice.setup.prohibitPHR_Timer =
    NR_MAC_MainConfig__phr_Config__setup__prohibitPHR_Timer_sf200; // sf20 = 20 subframes // NR_MAC_MainConfig__phr_Config__setup__prohibitPHR_Timer_sf1000
  mac_MainConfig->phr_Config->choice.setup.dl_PathlossChange = NR_MAC_MainConfig__phr_Config__setup__dl_PathlossChange_dB3;  // Value dB1 =1 dB, dB3 = 3 dB
  /* CDRX Configuration */
  ...
  *sr_ProhibitTimer_r9 = 0;   // SR tx on PUCCH, Value in number of SR period(s). Value 0 = no timer for SR, Value 2 = 2*SR
  mac_MainConfig->ext1 = CALLOC(1, sizeof(struct NR_MAC_MainConfig__ext1));
  ...
  if (*physicalConfigDedicated) {
    if ((*physicalConfigDedicated)->antennaInfo) {
      ...
      if (rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode==NR_AntennaInfoDedicated__transmissionMode_tm3) {
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction=
          CALLOC(1,sizeof(NR_AntennaInfoDedicated__codebookSubsetRestriction_PR));
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction->present =
          NR_AntennaInfoDedicated__codebookSubsetRestriction_PR_n2TxAntenna_tm3;
      ...
      } else if (rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode==NR_AntennaInfoDedicated__transmissionMode_tm4) {
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction=
          CALLOC(1,sizeof(NR_AntennaInfoDedicated__codebookSubsetRestriction_PR));
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction->present =
          NR_AntennaInfoDedicated__codebookSubsetRestriction_PR_n2TxAntenna_tm4;
      ...
      } else if (rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode==NR_AntennaInfoDedicated__transmissionMode_tm5) {
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction=
          CALLOC(1,sizeof(NR_AntennaInfoDedicated__codebookSubsetRestriction_PR));
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction->present =
          NR_AntennaInfoDedicated__codebookSubsetRestriction_PR_n2TxAntenna_tm5;
      ...
      } else if (rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode==NR_AntennaInfoDedicated__transmissionMode_tm6) {
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction=
          CALLOC(1,sizeof(NR_AntennaInfoDedicated__codebookSubsetRestriction_PR));
        (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.codebookSubsetRestriction->present =
          NR_AntennaInfoDedicated__codebookSubsetRestriction_PR_n2TxAntenna_tm6;
    ...
    if ((*physicalConfigDedicated)->cqi_ReportConfig != NULL) {
      if ((rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode == NR_AntennaInfoDedicated__transmissionMode_tm4) ||
          (rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode == NR_AntennaInfoDedicated__transmissionMode_tm5) ||
          (rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode == NR_AntennaInfoDedicated__transmissionMode_tm6)) {
        // feedback mode needs to be set as well
        // TODO: I think this is taken into account in the PHY automatically based on the transmission mode variable
        LOG_I(RRC, "Setting cqi reporting mode to rm31 (hardcoded)\n");
        *((*physicalConfigDedicated)->cqi_ReportConfig->cqi_ReportModeAperiodic) = NR_CQI_ReportModeAperiodic_rm31; // HLC CQI, single PMI
      }
    } else {
      LOG_E(RRC,"cqi_ReportConfig not present in physical_config_dedicated. Not reconfiguring!\n");
    }
  } else {
    LOG_E(RRC,"physical_config_dedicated not present in NR_RRCReconfiguration. Not reconfiguring!\n");
  }
  ...
  if (ue_context_pP->ue_context.does_nr) {
    MeasId6 = calloc(1, sizeof(NR_MeasIdToAddMod_t));
  ...
  ue_context_pP->ue_context.measurement_info->cellIndividualOffset[0] = NR_Q_OffsetRange_dB0;

  /* TODO: Extend to multiple carriers */
  // Add adjacent cell lists (max 6 per gNB)
  ...
  if (ue_context_pP->ue_context.does_nr) {
    MeasObj2 = calloc(1, sizeof(NR_MeasObjectToAddMod_t));

    if (MeasObj2 == NULL) exit(1);

    MeasObj2->measObjectId = 2;
    MeasObj2->measObject.present = NR_MeasObjectToAddMod__measObject_PR_measObjectNR_r15;
    ...
    MeasObj2->measObject.choice.measObjectNR_r15.rs_ConfigSSB_r15.measTimingConfig_r15.periodicityAndOffset_r15.present = NR_MTC_SSB_NR_r15__periodicityAndOffset_r15_PR_sf20_r15;
    ...
    MeasObj2->measObject.choice.measObjectNR_r15.rs_ConfigSSB_r15.measTimingConfig_r15.ssb_Duration_r15 = NR_MTC_SSB_NR_r15__ssb_Duration_r15_sf4;
    MeasObj2->measObject.choice.measObjectNR_r15.rs_ConfigSSB_r15.subcarrierSpacingSSB_r15 = NR_RS_ConfigSSB_NR_r15__subcarrierSpacingSSB_r15_kHz30;
    MeasObj2->measObject.choice.measObjectNR_r15.quantityConfigSet_r15 = 1;
    MeasObj2->measObject.choice.measObjectNR_r15.ext1 = calloc(1, sizeof(struct NR_MeasObjectNR_r15__ext1));

    if (MeasObj2->measObject.choice.measObjectNR_r15.ext1 == NULL) exit(1);

    MeasObj2->measObject.choice.measObjectNR_r15.ext1->bandNR_r15 = calloc(1, sizeof(struct NR_MeasObjectNR_r15__ext1__bandNR_r15));

    if (MeasObj2->measObject.choice.measObjectNR_r15.ext1->bandNR_r15 == NULL) exit(1);

    MeasObj2->measObject.choice.measObjectNR_r15.ext1->bandNR_r15->present = NR_MeasObjectNR_r15__ext1__bandNR_r15_PR_setup;
  ...
  LOG_I(RRC, "[gNB %d] frame %d: requesting A2, A3, A4, and A5 event reporting\n",
        ctxt_pP->module_id, ctxt_pP->frame);
  ReportConfig_A2->reportConfigId = 3;
  ReportConfig_A2->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigEUTRA;
  ReportConfig_A2->reportConfig.choice.reportConfigEUTRA.triggerType.present =
  ...
  if (ue_context_pP->ue_context.does_nr) {
    ...
    ReportConfig_NR->reportConfig.present = NR_ReportConfigToAddMod__reportConfig_PR_reportConfigInterRAT;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.triggerType.present = NR_ReportConfigInterRAT__triggerType_PR_event;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.triggerType.choice.event.eventId.present = NR_ReportConfigInterRAT__triggerType__event__eventId_PR_eventB1_NR_r15;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.triggerType.choice.event.eventId.choice.eventB1_NR_r15.b1_ThresholdNR_r15.present = NR_ThresholdNR_r15_PR_nr_RSRP_r15;
    ...
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.triggerType.choice.event.timeToTrigger = NR_TimeToTrigger_ms80;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.maxReportCells = 4;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.reportInterval = NR_ReportInterval_ms120;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.reportAmount = NR_ReportConfigInterRAT__reportAmount_infinity;
    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.ext7 = calloc(1, sizeof(struct NR_ReportConfigInterRAT__ext7));

    if (ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.ext7 == NULL) exit(1);

    ReportConfig_NR->reportConfig.choice.reportConfigInterRAT.ext7->reportQuantityCellNR_r15 = calloc(1, sizeof(struct NR_ReportQuantityNR_r15));
  ...
  *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRQ = NR_FilterCoefficient_fc4;
  ...
  size = do_RRCReconfiguration(ctxt_pP,
                                         ...
                                         (NR_DRB_ToReleaseList_t *) NULL, // DRB2_list,
                                         (struct NR_SPS_Config *) NULL,   // *sps_Config,
                                         (struct NR_PhysicalConfigDedicated *) *physicalConfigDedicated,
                                         measurements_enabled ? (NR_MeasObjectToAddModList_t *) MeasObj_list : NULL,
                                         measurements_enabled ? (NR_ReportConfigToAddModList_t *) ReportConfig_list : NULL,
                                         measurements_enabled ? (NR_QuantityConfig_t *) quantityConfig : NULL,
                                         measurements_enabled ? (NR_MeasIdToAddModList_t *) MeasId_list : NULL,
                                         (NR_MAC_MainConfig_t *) mac_MainConfig,
                                         (NR_MeasGapConfig_t *) NULL,
                                         (NR_MobilityControlInfo_t *) NULL,
                                         (NR_SecurityConfigHO_t *) NULL,
                                         (struct NR_MeasConfig__speedStatePars *) Sparams,
                                         (NR_RSRP_Range_t *) rsrp,
                                         (NR_C_RNTI_t *) cba_RNTI,
                                         (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *) dedicatedInfoNASList,
                                         (NR_SL_CommConfig_r12_t *) NULL,
                                         (NR_SL_DiscConfig_r12_t *) NULL,
                                         (NR_SCellToAddMod_r10_t *) NULL
  ...
  LOG_I(RRC, "[gNB %d] Frame %d, Logical Channel DL-DCCH, Generate NR_RRCReconfiguration (bytes %d, UE id %x)\n",
        ctxt_pP->module_id,
        ctxt_pP->frame,
        size,
        ue_context_pP->ue_context.rnti);
  LOG_D(RRC, "[FRAME %05d][RRC_gNB][MOD %u][][--- PDCP_DATA_REQ/%d Bytes (rrcReconfiguration to UE %x MUI %d) --->][PDCP][MOD %u][RB %u]\n",
        ctxt_pP->frame,
        ctxt_pP->module_id,
        size,
        ue_context_pP->ue_context.rnti,
  ...
  rrc_pdcp_config_asn1_req(ctxt_pP,
                          ...
                          *DRB_configList,
                          ...
                          NULL,
                          NULL
#if (NR_RRC_VERSION >= MAKE_VERSION(9, 0, 0))
                          , (NR_PMCH_InfoList_r9_t *) NULL
#endif
                          , NULL);
...
//-----------------------------------------------------------------------------
void
flexran_rrc_gNB_generate_defaultRRCReconfiguration(const protocol_ctxt_t *const ctxt_pP,
    rrc_gNB_ue_context_t          *const ue_context_pP,
    const uint8_t                ho_state)
//-----------------------------------------------------------------------------
{
  ...
  cba_RNTI = CALLOC(1, sizeof(NR_C_RNTI_t));
  ...
  if (rrc_inst->num_active_cba_groups) {
    ...
    cba_RNTI->buf[1] = 0xff;
    LOG_D(RRC, "[gNB %d] Frame %d: cba_RNTI = %x in group %d is attribued to UE %d\n",
          gnb_mod_idP,
          frameP,
    ...
    cba_RNTI->buf[0] = 0x0;
    cba_RNTI->buf[1] = 0x0;
    LOG_D(RRC, "[gNB %d] Frame %d: no cba_RNTI is configured for UE %d\n",
          gnb_mod_idP,
          frameP,
          ue_mod_idP);
  }
  ...
  *sr_ProhibitTimer_r9 = 0;   // SR tx on PUCCH, Value in number of SR period(s). Value 0 = no timer for SR, Value 2 = 2*SR
  mac_MainConfig->ext1 = CALLOC(1, sizeof(struct NR_MAC_MainConfig__ext1));
  mac_MainConfig->ext1->sr_ProhibitTimer_r9 = sr_ProhibitTimer_r9;
  // free the old NR_MAC_MainConfig_t: get a pointer to "old" memory, assign
  // the new values in the ue_context, then free it
  // Note: can not completely avoid race condition with FlexRAN
  NR_MAC_MainConfig_t *old_mac_MainConfig = ue_context_pP->ue_context.mac_MainConfig;
  ue_context_pP->ue_context.mac_MainConfig = mac_MainConfig;
  ...
  if (rrc_inst->carrier[0].sib1->tdd_Config!=NULL) {
    MeasObj->measObject.choice.measObjectEUTRA.ext1 = CALLOC(1, sizeof(struct NR_MeasObjectEUTRA__ext1));
    MeasObj->measObject.choice.measObjectEUTRA.ext1->measCycleSCell_r10 = NULL;
    MeasObj->measObject.choice.measObjectEUTRA.ext1->measSubframePatternConfigNeigh_r10 = CALLOC(1, sizeof(struct NR_MeasSubframePatternConfigNeigh_r10));
    MeasObj->measObject.choice.measObjectEUTRA.ext1->measSubframePatternConfigNeigh_r10->present=NR_MeasSubframePatternConfigNeigh_r10_PR_setup;
    MeasObj->measObject.choice.measObjectEUTRA.ext1->measSubframePatternConfigNeigh_r10->choice.setup.measSubframePatternNeigh_r10.present=NR_MeasSubframePattern_r10_PR_subframePatternTDD_r10;
    MeasObj->measObject.choice.measObjectEUTRA.ext1->measSubframePatternConfigNeigh_r10->choice.setup.measSubframePatternNeigh_r10.choice.subframePatternTDD_r10.present=
      NR_MeasSubframePattern_r10__subframePatternTDD_r10_PR_subframeConfig1_5_r10;
  ...
  CellsToAddModList = MeasObj->measObject.choice.measObjectEUTRA.cellsToAddModList;
  //=======
  //  MeasObj->measObject.choice.measObjectEUTRA.offsetFreq = (NR_Q_OffsetRange_t *) CALLOC(1,sizeof(NR_Q_OffsetRange_t));
...
//-----------------------------------------------------------------------------
int
rrc_gNB_generate_RRCReconfiguration_SCell(
  const protocol_ctxt_t *const ctxt_pP,
  rrc_gNB_ue_context_t *const ue_context_pP,
...
//-----------------------------------------------------------------------------
{
  ...
  //   sCellIndexToAdd = rrc_find_free_SCell_index(gnb_mod_idP, ue_mod_idP, 1);
  //  if (RC.rrc[gnb_mod_idP]->sCell_config[ue_mod_idP][sCellIndexToAdd] ) {
  ...
  size = do_RRCReconfiguration(ctxt_pP,
                                         buffer,
                                         rrc_gNB_get_next_transaction_identifier(ctxt_pP->module_id),//Transaction_id,
                                         (NR_SRB_ToAddModList_t *)NULL,
                                         (NR_DRB_ToAddModList_t *)NULL,
                                         (NR_DRB_ToReleaseList_t *)NULL,
                                         (struct NR_SPS_Config *)NULL,
                                         (struct NR_PhysicalConfigDedicated *)NULL,
                                         (NR_MeasObjectToAddModList_t *)NULL,
                                         (NR_ReportConfigToAddModList_t *)NULL,
                                         (NR_QuantityConfig_t *)NULL,
                                         (NR_MeasIdToAddModList_t *)NULL,
                                         (NR_MAC_MainConfig_t *)NULL,
                                         (NR_MeasGapConfig_t *)NULL,
                                         (NR_MobilityControlInfo_t *)NULL,
                                         (NR_SecurityConfigHO_t *)NULL,
                                         (struct NR_MeasConfig__speedStatePars *)NULL,
                                         (NR_RSRP_Range_t *)NULL,
                                         (NR_C_RNTI_t *)NULL,
                                         (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *)NULL,
                                         (NR_SL_CommConfig_r12_t *)NULL,
                                         (NR_SL_DiscConfig_r12_t *)NULL,
...
static int encode_CG_ConfigInfo(
  ...
  NR_UE_CapabilityRAT_Container_t ue_cap_rat_container_MRDC;
  NR_UE_CapabilityRAT_Container_t ue_cap_rat_container_nr;
  ...
  ue_cap_rat_container_list->list.size = RAT_Container_count * sizeof(NR_UE_CapabilityRAT_Container_t);
  ue_cap_rat_container_list->list.array = calloc(RAT_Container_count,sizeof(NR_UE_CapabilityRAT_Container_t *));

  if(ue_context_pP->ue_context.UE_Capability_MRDC) {
    ue_cap_rat_container_list->list.array[0] = calloc(1,sizeof(NR_UE_CapabilityRAT_Container_t));
    memcpy(ue_cap_rat_container_list->list.array[0],&ue_cap_rat_container_MRDC,sizeof(NR_UE_CapabilityRAT_Container_t));
  }

  if(ue_context_pP->ue_context.UE_Capability_nr) {
    ue_cap_rat_container_list->list.array[1] = calloc(1,sizeof(NR_UE_CapabilityRAT_Container_t));
    memcpy(ue_cap_rat_container_list->list.array[1],&ue_cap_rat_container_nr,sizeof(NR_UE_CapabilityRAT_Container_t));
  }

  //this xer_fprint logs can be enabled for additional debugging logs
  //xer_fprint(stdout,&asn_DEF_NR_UE_CapabilityRAT_ContainerList,ue_cap_rat_container_list);
...
//-----------------------------------------------------------------------------
{
  ...
  if (measResults2->measId > 0 ) {
    if (ue_context_pP->ue_context.measResults == NULL) {
      ue_context_pP->ue_context.measResults = CALLOC(1, sizeof(NR_MeasResults_t));
    }
  ...
  if (measResults2->measId == 7) {
    if ((ue_context_pP->ue_context.Status != RRC_NR_NSA) && (ue_context_pP->ue_context.Status != RRC_NR_NSA_RECONFIGURED)) {
      ...
      if(is_en_dc_supported(ue_context_pP->ue_context.UE_Capability)) {
        ...
        LOG_I(RRC,
              "[gNB %d] frame %d subframe %d: UE rnti %x switching to NSA mode\n",
  ...
  if (ue_context_pP->ue_context.Status != RRC_HO_EXECUTION) {
    ...
    derive_kgNB_star(ue_context_pP->ue_context.kgnb, X2AP_HANDOVER_REQ(msg).target_physCellId, earfcn_dl, true, KgNB_star);
    memcpy(X2AP_HANDOVER_REQ(msg).kgnb, KgNB_star, 32);
    X2AP_HANDOVER_REQ(msg).kgnb_ncc = ue_context_pP->ue_context.kgnb_ncc;
    ...
    for (int i=0; i<ue_context_pP->ue_context.setup_e_rabs; i++) {
      ...
      X2AP_HANDOVER_REQ(msg).e_rabs_tobesetup[i].gNB_addr = ue_context_pP->ue_context.e_rab[i].param.sgw_addr;
    ...
    LOG_D(RRC, "[gNB %d] Frame %d: Ignoring MeasReport from UE %x as Handover is in progress... \n", ctxt_pP->module_id, ctxt_pP->frame,
          ctxt_pP->rnti);
  }
}
...
void rrc_gNB_process_x2_setup_request(int mod_id, x2ap_setup_req_t *m) {
...
void rrc_gNB_process_x2_setup_response(int mod_id, x2ap_setup_resp_t *m) {
  if (RC.rrc[mod_id]->num_neigh_cells > MAX_NUM_NEIGH_CELLs) {
    LOG_E(RRC, "Error: number of neighbouring cells is exceeded \n");
    return;
  }
...
void rrc_gNB_process_handoverPreparationInformation(int mod_id, x2ap_handover_req_t *m) {
  ...
  LOG_D(RRC, "gNB %d: Created new UE context uid %u\n", mod_id, ue_context_target_p->local_uid);
  ...
  if ( LOG_DEBUGFLAG(DEBUG_ASN1) ) {
    xer_fprint(stdout, &asn_DEF_NR_UE_EUTRA_Capability, ue_context_target_p->ue_context.UE_Capability);
  }
  ...
  for (i = 0; i < ue_context_target_p->ue_context.nb_of_e_rabs; i++) {
    ...
    ue_context_target_p->ue_context.e_rab[i].param.sgw_addr = m->e_rabs_tobesetup[i].gNB_addr;
    ue_context_target_p->ue_context.e_rab[i].param.gtp_teid= m->e_rabs_tobesetup[i].gtp_teid;
    LOG_I(RRC, "gNB %d: Update the UE context after HO, e_rab_id %u gtp_teid %u\n", mod_id,
          ue_context_target_p->ue_context.e_rab[i].param.e_rab_id,
          ue_context_target_p->ue_context.e_rab[i].param.gtp_teid);
  }

  rrc_gNB_process_X2AP_TUNNEL_SETUP_REQ(mod_id, ue_context_target_p);
  ue_context_target_p->ue_context.Status = RRC_HO_EXECUTION;
  ue_context_target_p->ue_context.handover_info->state = HO_ACK;
}

void rrc_gNB_process_handoverCommand(
  int                         mod_id,
  struct rrc_gNB_ue_context_s *ue_context,
  x2ap_handover_req_ack_t     *m) {
  asn_dec_rval_t dec_rval;
  NR_HandoverCommand_t *ho = NULL;
  dec_rval = uper_decode(
               NULL,
  ...
  if (dec_rval.code != RC_OK ||
      ho->criticalExtensions.present != NR_HandoverCommand__criticalExtensions_PR_c1 ||
      ho->criticalExtensions.choice.c1.present != NR_HandoverCommand__criticalExtensions__c1_PR_handoverCommand_r8) {
...
void rrc_gNB_handover_ue_context_release(
  protocol_ctxt_t *const ctxt_pP,
  struct rrc_gNB_ue_context_s *ue_context_p) {
  int e_rab = 0;
  //MessageDef *msg_release_p = NULL;
  MessageDef *msg_delete_tunnels_p = NULL;
  uint32_t gNB_ue_s1ap_id = ue_context_p->ue_context.gNB_ue_s1ap_id;
  ...
  s1ap_ue_context_release(ctxt_pP->instance, ue_context_p->ue_context.gNB_ue_s1ap_id);
  //MSC_LOG_TX_MESSAGE(MSC_RRC_ENB, MSC_GTPU_ENB, NULL,0, "0 GTPV1U_ENB_DELETE_TUNNEL_REQ rnti %x ", gNB_ue_s1ap_id);
  ...
  for (e_rab = 0; e_rab < ue_context_p->ue_context.nb_of_e_rabs; e_rab++) {
    GTPV1U_ENB_DELETE_TUNNEL_REQ(msg_delete_tunnels_p).eps_bearer_id[GTPV1U_ENB_DELETE_TUNNEL_REQ(msg_delete_tunnels_p).num_erab++] =
      ue_context_p->ue_context.gnb_gtp_ebi[e_rab];
    // erase data
    ue_context_p->ue_context.gnb_gtp_teid[e_rab] = 0;
    memset(&ue_context_p->ue_context.gnb_gtp_addrs[e_rab], 0, sizeof(ue_context_p->ue_context.gnb_gtp_addrs[e_rab]));
    ue_context_p->ue_context.gnb_gtp_ebi[e_rab] = 0;
  }

  itti_send_msg_to_task(TASK_GTPV1_U, ctxt_pP->module_id, msg_delete_tunnels_p);
  struct rrc_ue_s1ap_ids_s *rrc_ue_s1ap_ids = NULL;
  rrc_ue_s1ap_ids = rrc_gNB_S1AP_get_ue_ids(RC.rrc[ctxt_pP->module_id], 0, gNB_ue_s1ap_id);

  if (rrc_ue_s1ap_ids != NULL) {
    rrc_gNB_S1AP_remove_ue_ids(RC.rrc[ctxt_pP->module_id], rrc_ue_s1ap_ids);
  }
}

/* This function may be incorrect. */
void rrc_gNB_handover_cancel(
  protocol_ctxt_t              *const ctxt_pP,
  struct rrc_gNB_ue_context_s  *ue_context_p) {
  int s1_cause = 1;                        /* 1 = tx2relocoverall-expiry */
  rrc_gNB_send_S1AP_UE_CONTEXT_RELEASE_REQ(ctxt_pP->module_id, ue_context_p,
      S1AP_CAUSE_RADIO_NETWORK, s1_cause);
}


int
flexran_rrc_gNB_trigger_handover (int mod_id,
                                  const protocol_ctxt_t *const ctxt_pP,
                                  rrc_gNB_ue_context_t  *ue_context_pP,
                                  int target_cell_id) {
  uint32_t earfcn_dl;
  ...
  /* Check if gNB id belongs to the supported ones-Extend for multiple carrieres */
  ...
  /* Check if gNB id was found */
  if (!cell_found) {
    LOG_E(RRC, "%s(): cannot find target gNB with phyCellId %d\n", __func__, target_cell_id);
    return -1;
  }
  ...
  RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(RC.rrc[ctxt_pP->module_id]->rrc_ue_head)) {
    ...
    if (ue_context_p->ue_context.Status == RRC_HO_EXECUTION && ue_context_p->ue_context.handover_info != NULL) {
      ...
      if (ue_context_p->ue_context.handover_info->state == HO_PREPARE) {
        LOG_D(RRC,
              "[gNB %d] Frame %d: Incoming handover detected for new UE_id %x) \n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              ctxt_pP->rnti);
        // source gNB generates rrcreconfiguration to prepare the HO
        LOG_I(RRC,
              "[gNB %d] Frame %d : Logical Channel UL-DCCH, processing RRCHandoverPreparationInformation, sending RRCReconfiguration to UE %d \n",
        ...
        LOG_I(RRC, "RRC Sends RRCReconfiguration to UE %d  at frame %d and subframe %d \n", ue_context_p->ue_context.rnti, ctxt_pP->frame,ctxt_pP->subframe);
      }
      ...
      if (ue_context_p->ue_context.handover_info->state == HO_ACK) {
        ...
        rrc_gNB_generate_HO_RRCReconfiguration(ctxt_pP, ue_context_p, X2AP_HANDOVER_REQ_ACK(msg).rrc_buffer,
            &X2AP_HANDOVER_REQ_ACK(msg).rrc_buffer_size);
        rrc_gNB_configure_rbs_handover(ue_context_p,ctxt_pP);
        ...
        for (int i=0; i<ue_context_p->ue_context.setup_e_rabs; i++) {
          ...
          X2AP_HANDOVER_REQ_ACK(msg).e_rabs_tobesetup[i].gtp_teid = ue_context_p->ue_context.gnb_gtp_x2u_teid[i];
          X2AP_HANDOVER_REQ_ACK(msg).e_rabs_tobesetup[i].gNB_addr = ue_context_p->ue_context.gnb_gtp_x2u_addrs[i];
        }

        itti_send_msg_to_task(TASK_X2AP, ENB_MODULE_ID_TO_INSTANCE(ctxt_pP->module_id), msg);
        LOG_I(RRC, "RRC Sends X2 HO ACK to the source gNB at frame %d and subframe %d \n", ctxt_pP->frame,ctxt_pP->subframe);
      }
    }
          ...
          switch (ITTI_MSG_ID(msg_p)) {
            case GTPV1U_ENB_DATA_FORWARDING_IND:
              PROTOCOL_CTXT_SET_BY_MODULE_ID(
                ...
                GTPV1U_ENB_DATA_FORWARDING_IND (msg_p).gnb_flag,
                GTPV1U_ENB_DATA_FORWARDING_IND (msg_p).rnti,
                GTPV1U_ENB_DATA_FORWARDING_IND (msg_p).frame,
                0,
                GTPV1U_ENB_DATA_FORWARDING_IND (msg_p).gNB_index);
              ...
              if (result != TRUE) {
                LOG_E(RRC, "target gnb send data forwarding buffer to PDCP request failed!\n");
              } else {
                LOG_D(RRC, "target gnb send data forwarding buffer to PDCP!\n");
              }
          ...
          switch (ITTI_MSG_ID(msg_p)) {
            case GTPV1U_ENB_END_MARKER_IND:
              PROTOCOL_CTXT_SET_BY_MODULE_ID(
                ...
                GTPV1U_ENB_END_MARKER_IND (msg_p).gnb_flag,
                GTPV1U_ENB_END_MARKER_IND (msg_p).rnti,
                GTPV1U_ENB_END_MARKER_IND (msg_p).frame,
                0,
                GTPV1U_ENB_END_MARKER_IND (msg_p).gNB_index);
              ...
              if (result != TRUE) {
                LOG_E(RRC, "target gnb send spgw buffer to PDCP request failed!\n");
              } else {
                LOG_D(RRC, "target gnb send spgw buffer to PDCP!\n");
              }
...
void
rrc_gNB_generate_HO_RRCReconfiguration(const protocol_ctxt_t *const ctxt_pP,
    rrc_gNB_ue_context_t  *const ue_context_pP,
    uint8_t               *buffer,
    int                    *_size
    //const uint8_t        ho_state
...
//-----------------------------------------------------------------------------
{
  ...
  NR_PhysicalConfigDedicated_t      *physicalConfigDedicated2;
  // srb 1: for HO
  struct NR_SRB_ToAddMod            *SRB1_config                      = NULL;
  struct NR_SRB_ToAddMod__rlc_Config *SRB1_rlc_config                 = NULL;
  struct NR_SRB_ToAddMod__logicalChannelConfig *SRB1_lchan_config     = NULL;
  struct NR_LogicalChannelConfig__ul_SpecificParameters
  ...
  struct NR_MeasConfig__speedStatePars *Sparams                       = NULL;
  NR_QuantityConfig_t               *quantityConfig                   = NULL;
  NR_MobilityControlInfo_t          *mobilityInfo                     = NULL;
  NR_SecurityConfigHO_t             *securityConfigHO                 = NULL;
  NR_CellsToAddMod_t                *CellToAdd                        = NULL;
  NR_CellsToAddModList_t            *CellsToAddModList                = NULL;
  struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *dedicatedInfoNASList = NULL;
  NR_DedicatedInfoNAS_t             *dedicatedInfoNas                 = NULL;
  ...
  LOG_D(RRC, "[gNB %d] Frame %d : handover preparation: add target gNB SRB1 and PHYConfigDedicated reconfiguration\n",
        ctxt_pP->module_id, ctxt_pP->frame);
  ...
  SRB1_rlc_config->present = NR_SRB_ToAddMod__rlc_Config_PR_explicitValue;
  SRB1_rlc_config->choice.explicitValue.present = NR_RLC_Config_PR_am;
  SRB1_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.t_PollRetransmit = NR_T_PollRetransmit_ms15;
  SRB1_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.pollPDU = NR_PollPDU_p8;
  SRB1_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.pollByte = NR_PollByte_kB1000;
  SRB1_rlc_config->choice.explicitValue.choice.am.ul_AM_RLC.maxRetxThreshold = NR_UL_AM_RLC__maxRetxThreshold_t16;
  SRB1_rlc_config->choice.explicitValue.choice.am.dl_AM_RLC.t_Reordering = NR_T_Reordering_ms35;
  SRB1_rlc_config->choice.explicitValue.choice.am.dl_AM_RLC.t_StatusProhibit = NR_T_StatusProhibit_ms10;
  SRB1_lchan_config = CALLOC(1, sizeof(*SRB1_lchan_config));
  SRB1_config->logicalChannelConfig = SRB1_lchan_config;
  SRB1_lchan_config->present = NR_SRB_ToAddMod__logicalChannelConfig_PR_explicitValue;
  ...
  *maxHARQ_Tx = NR_MAC_MainConfig__ul_SCH_Config__maxHARQ_Tx_n5;
  mac_MainConfig->ul_SCH_Config->maxHARQ_Tx = maxHARQ_Tx;
  periodicBSR_Timer = CALLOC(1, sizeof(long));
  *periodicBSR_Timer = NR_PeriodicBSR_Timer_r12_sf64;
  mac_MainConfig->ul_SCH_Config->periodicBSR_Timer = periodicBSR_Timer;
  mac_MainConfig->ul_SCH_Config->retxBSR_Timer = NR_RetxBSR_Timer_r12_sf320;
  mac_MainConfig->ul_SCH_Config->ttiBundling = 0; // FALSE
  mac_MainConfig->timeAlignmentTimerDedicated = NR_TimeAlignmentTimer_infinity;
  mac_MainConfig->drx_Config = NULL;
  mac_MainConfig->phr_Config = CALLOC(1, sizeof(*mac_MainConfig->phr_Config));
  mac_MainConfig->phr_Config->present = NR_MAC_MainConfig__phr_Config_PR_setup;
  mac_MainConfig->phr_Config->choice.setup.periodicPHR_Timer = NR_MAC_MainConfig__phr_Config__setup__periodicPHR_Timer_sf20; // sf20 = 20 subframes
  mac_MainConfig->phr_Config->choice.setup.prohibitPHR_Timer = NR_MAC_MainConfig__phr_Config__setup__prohibitPHR_Timer_sf20; // sf20 = 20 subframes
  mac_MainConfig->phr_Config->choice.setup.dl_PathlossChange = NR_MAC_MainConfig__phr_Config__setup__dl_PathlossChange_dB1;  // Value dB1 =1 dB, dB3 = 3 dB
  sr_ProhibitTimer_r9 = CALLOC(1, sizeof(long));
  *sr_ProhibitTimer_r9 = 0;   // SR tx on PUCCH, Value in number of SR period(s). Value 0 = no timer for SR, Value 2= 2*SR
  mac_MainConfig->ext1 = CALLOC(1, sizeof(struct NR_MAC_MainConfig__ext1));
  mac_MainConfig->ext1->sr_ProhibitTimer_r9 = sr_ProhibitTimer_r9;
  ...
  *physicalConfigDedicated = physicalConfigDedicated2;
  ...
  physicalConfigDedicated2->pdsch_ConfigDedicated->p_a = NR_PDSCH_ConfigDedicated__p_a_dB0;
  // PUCCH
  physicalConfigDedicated2->pucch_ConfigDedicated->ackNackRepetition.present =
    NR_PUCCH_ConfigDedicated__ackNackRepetition_PR_release;
  ...
  physicalConfigDedicated2->uplinkPowerControlDedicated->deltaMCS_Enabled =
    NR_UplinkPowerControlDedicated__deltaMCS_Enabled_en1;
  ...
  *physicalConfigDedicated2->uplinkPowerControlDedicated->filterCoefficient = NR_FilterCoefficient_fc4;  // fc4 dB
  // TPC-PDCCH-Config
  physicalConfigDedicated2->tpc_PDCCH_ConfigPUCCH->present = NR_TPC_PDCCH_Config_PR_setup;
  physicalConfigDedicated2->tpc_PDCCH_ConfigPUCCH->choice.setup.tpc_Index.present = NR_TPC_Index_PR_indexOfFormat3;
  ...
  physicalConfigDedicated2->tpc_PDCCH_ConfigPUSCH->present = NR_TPC_PDCCH_Config_PR_setup;
  physicalConfigDedicated2->tpc_PDCCH_ConfigPUSCH->choice.setup.tpc_Index.present = NR_TPC_Index_PR_indexOfFormat3;
  ...
  physicalConfigDedicated2->antennaInfo = CALLOC(1, sizeof(*physicalConfigDedicated2->antennaInfo));
  physicalConfigDedicated2->antennaInfo->present = NR_PhysicalConfigDedicated__antennaInfo_PR_explicitValue;
  //if ((*physicalConfigDedicated)->antennaInfo) {
  (*physicalConfigDedicated)->antennaInfo->choice.explicitValue.transmissionMode = rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode;
  LOG_D(RRC,"Setting transmission mode to %ld+1\n",rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode);
  ...
  physicalConfigDedicated2->antennaInfo->choice.explicitValue.ue_TransmitAntennaSelection.present =
    NR_AntennaInfoDedicated__ue_TransmitAntennaSelection_PR_release;
  ...
  physicalConfigDedicated2->cqi_ReportConfig->cqi_ReportPeriodic->present =  NR_CQI_ReportPeriodic_PR_release;
  ...
  if ((rrc_inst->configuration.radioresourceconfig[0].ue_TransmissionMode==NR_AntennaInfoDedicated__transmissionMode_tm4) ||
    ...
    *((*physicalConfigDedicated)->cqi_ReportConfig->cqi_ReportModeAperiodic)=NR_CQI_ReportModeAperiodic_rm31;
  } else {
    *physicalConfigDedicated2->cqi_ReportConfig->cqi_ReportModeAperiodic= NR_CQI_ReportModeAperiodic_rm30;
  }
  ...
  physicalConfigDedicated2->schedulingRequestConfig->present = NR_SchedulingRequestConfig_PR_setup;
  ...
  physicalConfigDedicated2->schedulingRequestConfig->choice.setup.dsr_TransMax =
    NR_SchedulingRequestConfig__setup__dsr_TransMax_n4;
  LOG_D(RRC,
        "handover_config [FRAME %05d][RRC_gNB][MOD %02d][][--- MAC_CONFIG_REQ  (SRB1 UE %x) --->][MAC_gNB][MOD %02d][]\n",
  ...
  rrc_mac_config_req_gNB(
    ...
    0,0,0,0,0,0,
    ue_context_pP->ue_context.rnti,
    (NR_BCCH_BCH_Message_t *) NULL,
    (NR_RadioResourceConfigCommonSIB_t *) NULL,
    (NR_RadioResourceConfigCommonSIB_t *) NULL,
    ue_context_pP->ue_context.physicalConfigDedicated,
    (NR_SCellToAddMod_r10_t *)NULL,
    (NR_MeasObjectToAddMod_t **) NULL,
    ue_context_pP->ue_context.mac_MainConfig,
    1,
    NULL,//SRB1_logicalChannelConfig,
    ue_context_pP->ue_context.measGapConfig,
    (NR_TDD_Config_t *) NULL,
    (NR_MobilityControlInfo_t *) NULL,
    (NR_SchedulingInfoList_t *) NULL,
    0,
    NULL,
    NULL,
    (NR_MBSFN_SubframeConfigList_t *) NULL,
    0, (NR_MBSFN_AreaInfoList_r9_t *) NULL, (NR_PMCH_InfoList_r9_t *) NULL,
    (NR_SystemInformationBlockType1_v1310_IEs_t *)NULL,
    0,
    (NR_BCCH_DL_SCH_Message_MBMS_t *) NULL,
    (NR_SchedulingInfo_MBMS_r14_t *) NULL,
    (struct NR_NonMBSFN_SubframeConfig_r14 *) NULL,
    (NR_SystemInformationBlockType1_MBMS_r14_t *) NULL,
    (NR_MBSFN_AreaInfoList_r9_t *) NULL,
    (NR_MBSFNAreaConfiguration_r9_t*) NULL
  );
  // Configure target gNB SRB2
  /// SRB2
  SRB2_config = CALLOC(1, sizeof(*SRB2_config));
  SRB_configList2 = CALLOC(1, sizeof(*SRB_configList2));
  memset(SRB_configList2, 0, sizeof(*SRB_configList2));
  ...
  *logicalchannelgroup = 0;
  ...
  ASN_SEQUENCE_ADD(&SRB_configList->list, SRB2_config);
  ASN_SEQUENCE_ADD(&(*SRB_configList2)->list, SRB2_config);
  // Configure target gNB DRB
  DRB_configList2 = CALLOC(1, sizeof(*DRB_configList2));
  /// DRB
  DRB_config = CALLOC(1, sizeof(*DRB_config));
  //DRB_config->drb_Identity = (NR_DRB_Identity_t) 1; //allowed values 1..32
  // NN: this is the 1st DRB for this ue, so set it to 1
  DRB_config->drb_Identity = (NR_DRB_Identity_t) 1;  // (ue_mod_idP+1); //allowed values 1..32
  DRB_config->logicalChannelIdentity = CALLOC(1, sizeof(long));
  ...
  *sr_ProhibitTimer_r9 = 0;   // SR tx on PUCCH, Value in number of SR period(s). Value 0 = no timer for SR, Value 2= 2*SR
  ...
  ReportConfig_A5->reportConfig.choice.reportConfigEUTRA.reportAmount = NR_ReportConfigEUTRA__reportAmount_infinity;
  ASN_SEQUENCE_ADD(&ReportConfig_list->list, ReportConfig_A5);
  //  rrcReconfiguration->criticalExtensions.choice.c1.choice.rrcReconfiguration_r8.measConfig->reportConfigToAddModList = ReportConfig_list;
  ...
  *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRQ = NR_FilterCoefficient_fc4;
  ...
  LOG_D(RRC, "[gNB %d] Frame %d: handover preparation: targetPhysCellId: %ld mod_id: %d ue: %x \n",
        ctxt_pP->module_id,
        ctxt_pP->frame,
  ...
  *mobilityInfo->additionalSpectrumEmission = (NR_AdditionalSpectrumEmission_t) 1;  //Check this value!
  mobilityInfo->t304 = NR_MobilityControlInfo__t304_ms200;    // need to configure an appropriate value here
  ...
  memcpy((void *)mobilityInfo->radioResourceConfigCommon.rach_ConfigCommon,
         (void *)&rrc_inst->carrier[0] /* CROUX TBC */.sib2->radioResourceConfigCommon.rach_ConfigCommon, sizeof(NR_RACH_ConfigCommon_t));
  ...
  memcpy((void *)mobilityInfo->radioResourceConfigCommon.pdsch_ConfigCommon,
         (void *)&rrc_inst->carrier[0] /* CROUX TBC */.sib2->radioResourceConfigCommon.pdsch_ConfigCommon, sizeof(NR_PDSCH_ConfigCommon_t));
  memcpy((void *)&mobilityInfo->radioResourceConfigCommon.pusch_ConfigCommon,
         (void *)&rrc_inst->carrier[0] /* CROUX TBC */.sib2->radioResourceConfigCommon.pusch_ConfigCommon, sizeof(NR_PUSCH_ConfigCommon_t));
  ...
  memcpy((void *)mobilityInfo->radioResourceConfigCommon.pucch_ConfigCommon,
         (void *)&rrc_inst->carrier[0] /* CROUX TBC */.sib2->radioResourceConfigCommon.pucch_ConfigCommon, sizeof(NR_PUCCH_ConfigCommon_t));
  ...
  memcpy((void *)mobilityInfo->radioResourceConfigCommon.soundingRS_UL_ConfigCommon,
         ...
         sizeof(NR_SoundingRS_UL_ConfigCommon_t));
  ...
  memcpy((void *)mobilityInfo->radioResourceConfigCommon.uplinkPowerControlCommon,
         ...
         sizeof(NR_UplinkPowerControlCommon_t));
  ...
  mobilityInfo->carrierBandwidth->dl_Bandwidth = NR_CarrierBandwidthEUTRA__dl_Bandwidth_n25;
  ...
  securityConfigHO->handoverType.present = NR_SecurityConfigHO__handoverType_PR_intraNR;
  securityConfigHO->handoverType.choice.intraNR.securityAlgorithmConfig = NULL; /* TODO: to be checked */
  securityConfigHO->handoverType.choice.intraNR.keyChangeIndicator = 0;
  securityConfigHO->handoverType.choice.intraNR.nextHopChainingCount = 0;
  /* Initialize NAS list */
  dedicatedInfoNASList = CALLOC(1, sizeof(struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList));
  ...
  int rrc_size;
  rrc_size = do_RRCReconfiguration(ctxt_pP,
             ...
             (NR_MAC_MainConfig_t *)mac_MainConfig,
             (NR_MeasGapConfig_t *)NULL,
             (NR_MobilityControlInfo_t *)mobilityInfo,
             (NR_SecurityConfigHO_t *)securityConfigHO,
             (struct NR_MeasConfig__speedStatePars *)Sparams,
             (NR_RSRP_Range_t *)rsrp,
             (NR_C_RNTI_t *)cba_RNTI,
             (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *)dedicatedInfoNASList,
  ...
  *_size = size = ho_size;
  ...
  LOG_I(RRC,
        "[gNB %d] Frame %d, Logical Channel DL-DCCH, Generate RRCReconfiguration handover (bytes %d, UE id %x)\n",
        ctxt_pP->module_id, ctxt_pP->frame, size, ue_context_pP->ue_context.rnti);
  LOG_D(RRC,
        "[FRAME %05d][RRC_gNB][MOD %u][][--- PDCP_DATA_REQ/%d Bytes (rrcReconfiguration handover to UE %x MUI %d) --->][PDCP][MOD %u][RB %u]\n",
        ctxt_pP->frame, ctxt_pP->module_id, size, ue_context_pP->ue_context.rnti, rrc_gNB_mui, ctxt_pP->module_id, DCCH);
  MSC_LOG_TX_MESSAGE(
    MSC_RRC_ENB,
    MSC_RRC_UE,
    buffer,
    size,
    MSC_AS_TIME_FMT" rrcReconfiguration handover UE %x MUI %d size %u",
    MSC_AS_TIME_ARGS(ctxt_pP),
    ue_context_pP->ue_context.rnti,
    rrc_gNB_mui,
    size);
...
void
rrc_gNB_configure_rbs_handover(struct rrc_gNB_ue_context_s *ue_context_p, protocol_ctxt_t *const ctxt_pP) {
  uint16_t                            Idx;
  Idx = DCCH;
#if 1
...
#endif
  LOG_I(RRC, "[gNB %d] CALLING RLC CONFIG SRB1 (rbid %d) for UE %x\n",
  ...
  rrc_pdcp_config_asn1_req(ctxt_pP,
                           ...
                           NULL,
                           NULL,
                           (NR_PMCH_InfoList_r9_t *) NULL, NULL);
  if (!NODE_IS_CU(RC.rrc[ctxt_pP->module_id]->node_type)) {
    rrc_rlc_config_asn1_req(ctxt_pP,
                            ue_context_p->ue_context.SRB_configList,
                            (NR_DRB_ToAddModList_t *) NULL,
  ...
  LOG_I(RRC, "rrc_gNB_target_add_ue_handover module_id %d rnti %d\n", ctxt_pP->module_id, ctxt_pP->rnti);
  ...
  rrc_mac_config_req_gNB(
    ...
    (NR_TDD_Config_t *) NULL,
    (NR_MobilityControlInfo_t *) ue_context_p->ue_context.mobilityInfo,
    (NR_SchedulingInfoList_t *) NULL,
    0,
    NULL,
    NULL,
    (NR_MBSFN_SubframeConfigList_t *) NULL, 0,
    (NR_MBSFN_AreaInfoList_r9_t *) NULL, (NR_PMCH_InfoList_r9_t *) NULL,
    (NR_SystemInformationBlockType1_v1310_IEs_t *)NULL,
    0,
    (NR_BCCH_DL_SCH_Message_MBMS_t *) NULL,
    (NR_SchedulingInfo_MBMS_r14_t *) NULL,
...
//-----------------------------------------------------------------------------
{
  ...
  if (SRB_configList != NULL) {
    for (int i = 0; (i < SRB_configList->list.count) && (i < 3); i++) {
      if (SRB_configList->list.array[i]->srb_Identity == 1) {
        ...
        ue_context_pP->ue_context.Srb2.Srb_info.Srb_id = 2;
        LOG_I(RRC,"[gNB %d] Frame  %d CC %d : SRB2 is now active\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              ue_context_pP->ue_context.primaryCC_id);
      } else {
        LOG_W(RRC,"[gNB %d] Frame  %d CC %d : invalide SRB identity %ld\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              ue_context_pP->ue_context.primaryCC_id,
  ...
  if (DRB_configList != NULL) {
    for (int i = 0; i < DRB_configList->list.count; i++) {  // num max DRB (11-3-8)
      if (DRB_configList->list.array[i]) {
        ...
        LOG_I(RRC, "[gNB %d] Frame  %d : Logical Channel UL-DCCH, Received NR_RRCReconfigurationComplete from UE rnti %x, reconfiguring DRB %d/LCID %d\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              ctxt_pP->rnti,
        ...
        /* For pre-ci tests */
        LOG_I(RRC, "[gNB %d] Frame  %d : Logical Channel UL-DCCH, Received NR_RRCReconfigurationComplete from UE %u, reconfiguring DRB %d/LCID %d\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              oai_emulation.info.gNB_ue_local_uid_to_ue_module_id[ctxt_pP->module_id][ue_context_pP->local_uid],
              (int)DRB_configList->list.array[i]->drb_Identity,
              (int)*DRB_configList->list.array[i]->logicalChannelIdentity);

        if (ue_context_pP->ue_context.DRB_active[drb_id] == 0) {
          ue_context_pP->ue_context.DRB_active[drb_id] = 1;
          LOG_D(RRC, "[gNB %d] Frame %d: Establish RLC UM Bidirectional, DRB %d Active\n",
                ctxt_pP->module_id, ctxt_pP->frame, (int)DRB_configList->list.array[i]->drb_Identity);

          if (!EPC_MODE_ENABLED && !ENB_NAS_USE_TUN) {
            LOG_I(OIP, "[gNB %d] trying to bring up the OAI interface oai%d\n",
                  ctxt_pP->module_id,
                  ctxt_pP->module_id);
            ...
            if (oip_ifup == 0) { // interface is up --> send a config the DRB
              ...
              dest_ip_offset = 8;
              LOG_I(OIP,
                    "[gNB %d] Config the oai%d to send/receive pkt on DRB %ld to/from the protocol stack\n",
                    ctxt_pP->module_id, ctxt_pP->module_id,
                    (long int)((ue_context_pP->local_uid * NR_maxDRB) + DRB_configList->list.array[i]->drb_Identity));
              ue_module_id = oai_emulation.info.gNB_ue_local_uid_to_ue_module_id[ctxt_pP->module_id][ue_context_pP->local_uid];
              rb_conf_ipv4(0, //add
                           ue_module_id,  //cx
                           ctxt_pP->module_id,    //inst
                           (ue_module_id * NR_maxDRB) + DRB_configList->list.array[i]->drb_Identity, // RB
                           0,    //dscp
              ...
              LOG_D(RRC, "[gNB %d] State = Attached (UE rnti %x module id %u)\n",
                    ctxt_pP->module_id, ue_context_pP->ue_context.rnti, ue_module_id);
            } /* oip_ifup */
          } /* !EPC_MODE_ENABLED && ENB_NAS_USE_TUN*/

          LOG_D(RRC,
                PROTOCOL_RRC_CTXT_UE_FMT" RRC_gNB --- MAC_CONFIG_REQ  (DRB) ---> MAC_gNB\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
          ...
          LOG_D(RRC, PROTOCOL_RRC_CTXT_UE_FMT" RRC_gNB --- MAC_CONFIG_REQ  (DRB) ---> MAC_gNB\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
          ...
          if (NODE_IS_MONOLITHIC(RC.rrc[ctxt_pP->module_id]->node_type)) {
            rrc_mac_config_req_gNB(ctxt_pP->module_id,
                                   ...
                                   ue_context_pP->ue_context.mac_MainConfig,
                                   DRB2LCHAN[i],
                                   (NR_LogicalChannelConfig_t *) NULL,
                                   (NR_MeasGapConfig_t *) NULL,
                                   (NR_TDD_Config_t *) NULL,
                                   NULL,
                                   (NR_SchedulingInfoList_t *) NULL,
  ...
  if (ue_context_pP->ue_context.does_nr &&
    ...
    rrc_gNB_generate_NR_UECapabilityEnquiry(ctxt_pP, ue_context_pP);
  }
}
  ...
  switch (RC.rrc[ctxt_pP->module_id]->node_type) {
    ...
    case ngran_gNB  :
          ...
          if ((*SRB_configList)->list.array[cnt]->srb_Identity == 1) {
            ...
            if (SRB1_config->logicalChannelConfig) {
              if (SRB1_config->logicalChannelConfig->present ==
                  NR_SRB_ToAddMod__logicalChannelConfig_PR_explicitValue) {
            ...
            if (RC.rrc[ctxt_pP->module_id]->node_type == ngran_eNB) {
              rrc_mac_config_req_eNB(ctxt_pP->module_id,
                                     ...
                                     NULL,
                                     (NR_SchedulingInfoList_t *) NULL,
                                     0, NULL, NULL, (NR_MBSFN_SubframeConfigList_t *) NULL,
                                     0, (NR_MBSFN_AreaInfoList_r9_t *) NULL, (NR_PMCH_InfoList_r9_t *) NULL,
                                     (NR_SystemInformationBlockType1_v1310_IEs_t *)NULL,
                                     0,
                                     (NR_BCCH_DL_SCH_Message_MBMS_t *) NULL,
      ...
      MSC_LOG_TX_MESSAGE(
        ...
        ue_p->Srb0.Tx_buffer.payload_size);
      LOG_I(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" [RAPROC] Logical Channel DL-CCCH, Generating RRCSetup (bytes %d)\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
            ue_p->Srb0.Tx_buffer.payload_size);
...
void setup_ngran_CU(gNB_RRC_INST *rrc) {
}

//-----------------------------------------------------------------------------
char openair_rrc_gNB_configuration(
  const module_id_t gnb_mod_idP,
...
//-----------------------------------------------------------------------------
{
  ...
  AssertFatal(RC.rrc[gnb_mod_idP] != NULL, "RC.rrc not initialized!");
  ...
  //        RC.rrc[gnb_mod_idP]->Srb2[j].Active = 0;
  //    }
  ...
  for (CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    // if we are here the RC.rrc[gnb_mod_idP]->MBMS_flag > 0,
    /// MCCH INIT
    ...
    openair_rrc_top_init_gNB(RC.rrc[ctxt.module_id]->carrier[CC_id].MBMS_flag,0);
  }
...
static int
is_en_dc_supported(
  NR_UE_EUTRA_Capability_t *c
)
//-----------------------------------------------------------------------------
{
  /* to be refined - check that the gNB is connected to a gNB, check that
...
#define NCE nonCriticalExtension
  return c != NULL
         ...
         NR_IRAT_ParametersNR_r15__en_DC_r15_supported;
#undef NCE
}
...
//-----------------------------------------------------------------------------
{
  ...
  NR_UL_DCCH_Message_t               *ul_dcch_msg = NULL; //&uldcchmsg;
  int i;
  struct rrc_gNB_ue_context_s        *ue_context_p = NULL;
  MessageDef                         *msg_delete_tunnels_p = NULL;
  uint8_t                             xid;
  ...
  if (ul_dcch_msg->message.present == NR_UL_DCCH_MessageType_PR_c1) {
    switch (ul_dcch_msg->message.choice.c1.present) {
      ...
      //TODO
      case NR_UL_DCCH_MessageType__c1_PR_ulInformationTransferMRDC:
        break;
      ...
      case NR_UL_DCCH_MessageType__c1_PR_measurementReport:
        ...
        LOG_D(RRC,
              ...
              "%d bytes (measurementReport) ---> RRC_gNB\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              DCCH,
              sdu_sizeP);
        rrc_gNB_process_MeasurementReport(
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcReconfigurationComplete:
        ...
        // to avoid segmentation fault
        if(!ue_context_p) {
          LOG_I(RRC, "Processing NR_RRCReconfigurationComplete UE %x, ue_context_p is NULL\n", ctxt_pP->rnti);
          break;
        }
        ...
        if (ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.criticalExtensions.
            present ==
            NR_RRCReconfigurationComplete__criticalExtensions_PR_rrcReconfigurationComplete_r8) {
          ...
          if (EPC_MODE_ENABLED) {
            if (ue_context_p->ue_context.Status == RRC_RECONFIGURED) {
              ...
              LOG_I(RRC,
                    ...
                    PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.rrc_TransactionIdentifier);
              //clear
              int16_t UE_id = find_UE_id(ctxt_pP->module_id, ctxt_pP->rnti);

              if(UE_id == -1) {
                LOG_E(RRC,
                      PROTOCOL_RRC_CTXT_UE_FMT" RRCReconfigurationComplete without rnti %x, fault\n",
                      PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),ctxt_pP->rnti);
                break;
              }
              ...
              //Looking for a condition to trigger S1AP E-RAB-Modification-indication, based on the reception of RRCReconfigurationComplete
              //including NR specific elements.
              if(ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.criticalExtensions.choice.rrcReconfigurationComplete_r8.
                      ...
                      if(ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.criticalExtensions.choice.rrcReconfigurationComplete_r8.
                        ...
                        if (ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.criticalExtensions.choice.rrcReconfigurationComplete_r8.
                          ...
                          if(ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.criticalExtensions.choice.rrcReconfigurationComplete_r8.
                            ...
                            rrc_gNB_send_E_RAB_Modification_Indication(ctxt_pP, ue_context_p);
                          }
                        }
                      }
                    }
        ...
        if (EPC_MODE_ENABLED) {
          if (dedicated_DRB == 1) {
            //    rrc_gNB_send_S1AP_E_RAB_SETUP_RESP(ctxt_pP,
            //               ue_context_p,
            //               ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.rrc_TransactionIdentifier);
            if (ue_context_p->ue_context.nb_of_modify_e_rabs > 0) {
              rrc_gNB_send_S1AP_E_RAB_MODIFY_RESP(ctxt_pP,
                                                  ue_context_p,
                                                  ul_dcch_msg->message.choice.c1.choice.rrcReconfigurationComplete.rrc_TransactionIdentifier);
              ...
              for(i = 0; i < NB_RB_MAX; i++) {
                if(xid == ue_context_p->ue_context.e_rab[i].xid) {
                  GTPV1U_ENB_DELETE_TUNNEL_REQ(msg_delete_tunnels_p).eps_bearer_id[GTPV1U_ENB_DELETE_TUNNEL_REQ(msg_delete_tunnels_p).num_erab++] = ue_context_p->ue_context.gnb_gtp_ebi[i];
                  ...
                  ue_context_p->ue_context.gnb_gtp_ebi[i]  = 0;
                }
              }

              itti_send_msg_to_task(TASK_GTPV1_U, ctxt_pP->instance, msg_delete_tunnels_p);
              //S1AP_E_RAB_RELEASE_RESPONSE
              rrc_gNB_send_S1AP_E_RAB_RELEASE_RESPONSE(ctxt_pP,
                  ue_context_p,
                  xid);
            } else {
              rrc_gNB_send_S1AP_E_RAB_SETUP_RESP(ctxt_pP,
            ...
            if(ue_context_p->ue_context.reestablishment_cause == NR_ReestablishmentCause_spare1) {
              rrc_gNB_send_S1AP_INITIAL_CONTEXT_SETUP_RESP(ctxt_pP,
                  ue_context_p);
            } else {
              ue_context_p->ue_context.reestablishment_cause = NR_ReestablishmentCause_spare1;
            ...
            LOG_I(RRC,"issue rrc_gNB_send_PATH_SWITCH_REQ \n");
            rrc_gNB_send_PATH_SWITCH_REQ(ctxt_pP,ue_context_p);
          }
        } /* EPC_MODE_ENABLED */
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcReestablishmentComplete:
        ...
        MSC_LOG_RX_MESSAGE(
          ...
          Rx_sdu,
          sdu_sizeP,
          MSC_AS_TIME_FMT" NR_RRCReestablishmentComplete UE %x size %u",
          MSC_AS_TIME_ARGS(ctxt_pP),
          ue_context_p->ue_context.rnti,
          sdu_sizeP);
        LOG_I(RRC,
              ...
              sdu_sizeP);
        {
          ...
          if (!ue_context_p) {
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" NR_RRCReestablishmentComplete without UE context, falt\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
            break;
          }
          ...
          if (ul_dcch_msg->message.choice.c1.choice.rrcReestablishmentComplete.criticalExtensions.present ==
              NR_RRCReestablishmentComplete__criticalExtensions_PR_rrcReestablishmentComplete_r8) {
            rrc_gNB_process_RRCReestablishmentComplete(ctxt_pP, reestablish_rnti, ue_context_p,
                ...
                &ul_dcch_msg->message.choice.c1.choice.rrcReestablishmentComplete.criticalExtensions.choice.rrcReestablishmentComplete_r8);
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcSetupComplete:
        ...
        // to avoid segmentation fault
        if(!ue_context_p) {
          LOG_I(RRC, "Processing NR_RRCSetupComplete UE %x, ue_context_p is NULL\n", ctxt_pP->rnti);
          break;
        }
        ...
        if (ul_dcch_msg->message.choice.c1.choice.rrcSetupComplete.criticalExtensions.present ==
            NR_RRCSetupComplete__criticalExtensions_PR_c1) {
          if (ul_dcch_msg->message.choice.c1.choice.rrcSetupComplete.criticalExtensions.choice.c1.
              present ==
              NR_RRCSetupComplete__criticalExtensions__c1_PR_rrcSetupComplete_r8) {
            AssertFatal(!NODE_IS_DU(RC.rrc[ctxt_pP->module_id]->node_type),
                        "should not be reached in DU\n");
            rrc_gNB_process_RRCSetupComplete(
              ctxt_pP,
              ue_context_p,
              &ul_dcch_msg->message.choice.c1.choice.rrcSetupComplete.criticalExtensions.choice.c1.choice.rrcSetupComplete_r8);
      ...
      case NR_UL_DCCH_MessageType__c1_PR_securityModeComplete:
        ...
        //rrc_pdcp_config_req (gnb_mod_idP, frameP, 1,CONFIG_ACTION_SET_SECURITY_MODE, (ue_mod_idP * NB_RB_MAX) + DCCH, 0x77);
      ...
      case NR_UL_DCCH_MessageType__c1_PR_securityModeFailure:
        ...
        //#warning "LG Removed rrc_gNB_generate_UECapabilityEnquiry after receiving securityModeFailure"
      ...
      case NR_UL_DCCH_MessageType__c1_PR_ueCapabilityInformation:
        ...
        LOG_D(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT" RLC RB %02d --- RLC_DATA_IND %d bytes "
              "(UECapabilityInformation) ---> RRC_gNB\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              DCCH,
              sdu_sizeP);
        ...
        if ( LOG_DEBUGFLAG(DEBUG_ASN1) ) {
          xer_fprint(stdout, &asn_DEF_NR_UE_EUTRA_Capability, ue_context_p->ue_context.UE_Capability);
        }
        ...
        if (EPC_MODE_ENABLED) {
          rrc_gNB_send_S1AP_UE_CAPABILITIES_IND(ctxt_pP,
                                                ue_context_p,
                                                ul_dcch_msg);
        } else {
        ...
        rrc_gNB_generate_defaultRRCReconfiguration(ctxt_pP,
            ue_context_p,
            RC.rrc[ctxt_pP->module_id]->HO_flag);
        break;

      case NR_UL_DCCH_MessageType__c1_PR_ulHandoverPreparationTransfer:
      ...
      case NR_UL_DCCH_MessageType__c1_PR_ulInformationTransfer:
        ...
        if (EPC_MODE_ENABLED == 1) {
          rrc_gNB_send_S1AP_UPLINK_NAS(ctxt_pP,
                                       ue_context_p,
                                       ul_dcch_msg);
        }
      ...
      case NR_UL_DCCH_MessageType__c1_PR_ueInformationResponse_r9:
        T(T_ENB_RRC_UE_INFORMATION_RESPONSE_R9, T_INT(ctxt_pP->module_id), T_INT(ctxt_pP->frame),
          T_INT(ctxt_pP->subframe), T_INT(ctxt_pP->rnti));
        break;

      case NR_UL_DCCH_MessageType__c1_PR_proximityIndication_r9:
        T(T_ENB_RRC_PROXIMITY_INDICATION_R9, T_INT(ctxt_pP->module_id), T_INT(ctxt_pP->frame),
          T_INT(ctxt_pP->subframe), T_INT(ctxt_pP->rnti));
        break;

      case NR_UL_DCCH_MessageType__c1_PR_rnReconfigurationComplete_r10:
        T(T_ENB_RRC_RECONFIGURATION_COMPLETE_R10, T_INT(ctxt_pP->module_id), T_INT(ctxt_pP->frame),
          T_INT(ctxt_pP->subframe), T_INT(ctxt_pP->rnti));
        break;

      case NR_UL_DCCH_MessageType__c1_PR_mbmsCountingResponse_r10:
        T(T_ENB_RRC_MBMS_COUNTING_RESPONSE_R10, T_INT(ctxt_pP->module_id), T_INT(ctxt_pP->frame),
          T_INT(ctxt_pP->subframe), T_INT(ctxt_pP->rnti));
    	LOG_E(RRC, "THINH [NR_UL_DCCH_MessageType__c1_PR_mbmsCountingResponse_r10]\n");

        break;

      case NR_UL_DCCH_MessageType__c1_PR_interFreqRSTDMeasurementIndication_r10:
    ...
    LOG_I(RRC, "[NR_UL_DCCH_MessageType_PR_messageClassExtension]\n");

    switch (ul_dcch_msg->message.choice.messageClassExtension.present) {
      case NR_UL_DCCH_MessageType__messageClassExtension_PR_NOTHING: /* No components present */
        break;

      case NR_UL_DCCH_MessageType__messageClassExtension_PR_c2: //SidelinkUEInformation
        if(ul_dcch_msg->message.choice.messageClassExtension.choice.c2.present ==
          NR_UL_DCCH_MessageType__messageClassExtension__c2_PR_scgFailureInformationNR_r15){
          if (ul_dcch_msg->message.choice.messageClassExtension.choice.c2.choice.scgFailureInformationNR_r15.
            criticalExtensions.present == NR_SCGFailureInformationNR_r15__criticalExtensions_PR_c1){
            if (ul_dcch_msg->message.choice.messageClassExtension.choice.c2.choice.scgFailureInformationNR_r15.criticalExtensions.
              choice.c1.present == NR_SCGFailureInformationNR_r15__criticalExtensions__c1_PR_scgFailureInformationNR_r15){
              if (ul_dcch_msg->message.choice.messageClassExtension.choice.c2.choice.scgFailureInformationNR_r15.criticalExtensions.
        ...
        else if(ul_dcch_msg->message.choice.messageClassExtension.choice.c2.present == NR_UL_DCCH_MessageType__messageClassExtension__c2_PR_sidelinkUEInformation_r12){
          //case UL_DCCH_MessageType__messageClassExtension__c2_PR_sidelinkUEInformation_r12: //SidelinkUEInformation
          LOG_I(RRC,"THINH [NR_UL_DCCH_MessageType__messageClassExtension_PR_c2]\n");
          LOG_DUMPMSG(RRC,DEBUG_RRC,(char *)Rx_sdu,sdu_sizeP,
          ...
          LOG_I(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" RLC RB %02d --- RLC_DATA_IND %d bytes "
            "(SidelinkUEInformation) ---> RRC_gNB\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
            DCCH,
            sdu_sizeP);
          rrc_gNB_process_SidelinkUEInformation(
            ctxt_pP,
            ue_context_p,
...
void rrc_gNB_reconfigure_DRBs (const protocol_ctxt_t *const ctxt_pP,
                               rrc_gNB_ue_context_t  *ue_context_pP) {
  int i;
  int e_rab_done=0;
  ...
  rrc_gNB_generate_dedicatedRRCReconfiguration(ctxt_pP, ue_context_pP, 0);
}
    ...
    for (int j=0; j<RC.nb_inst; j++) {
      ...
      if (rrc->configuration.mcc[0] == f1_setup_req->mcc[i] &&
        ...
        AssertFatal(dec_rval.code == RC_OK,
                    ...
                    j,
                    dec_rval.consumed );
        NR_BCCH_BCH_Message_t *mib = &rrc->carrier[0].mib;
        NR_BCCH_BCH_Message_t *mib_DU = rrc->carrier[0].mib_DU;
        ...
        NR_BCCH_DL_SCH_Message_t *bcch_message = rrc->carrier[0].siblock1_DU;
                                           ...
                                           int message_id)
{
  ...
  if (rrc_release_info.num_UEs > 0) {
    ...
    for (release_num = 0, release_ctrl = &rrc_release_info.RRC_release_ctrl[0];
      ...
      if (release_ctrl->flag == 1 && release_ctrl->rnti == rnti && release_ctrl->rrc_gNB_mui == message_id) {
        release_ctrl->flag = 3;
      ...
      if (release_ctrl->flag == 2 && release_ctrl->rnti == rnti && release_ctrl->rrc_gNB_mui == message_id) {
        release_ctrl->flag = 4;
...
void process_unsuccessful_rlc_sdu_indication(int instance, int rnti)
{
  ...
  for (release_num = 0; release_num < NUMBER_OF_UE_MAX; release_num++) {
    if (rrc_release_info.RRC_release_ctrl[release_num].flag == 0) {
      ...
      rrc_release_info.RRC_release_ctrl[release_num].rrc_gNB_mui = -1;     /* not defined */
      rrc_release_info.num_UEs++;
...
//-----------------------------------------------------------------------------
int add_ue_to_remove(struct rrc_gNB_ue_context_s **ue_to_be_removed,
...
void rrc_subframe_process(protocol_ctxt_t *const ctxt_pP, const int CC_id) {
  ...
  struct timeval ts;
  struct rrc_gNB_ue_context_s *ue_context_p = NULL;
  struct rrc_gNB_ue_context_s *ue_to_be_removed[NUMBER_OF_UE_MAX];
  ...
  RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(RC.rrc[ctxt_pP->module_id]->rrc_ue_head)) {
    ...
    if (ue_context_p->ue_context.ue_release_timer_s1 > 0) {
      ...
      if (ue_context_p->ue_context.ue_release_timer_s1 >= ue_context_p->ue_context.ue_release_timer_thres_s1) {
        ...
        if (EPC_MODE_ENABLED && !NODE_IS_DU(RC.rrc[ctxt_pP->module_id]->node_type))
          rrc_gNB_generate_RRCRelease(ctxt_pP, ue_context_p);
        else
          removed_ue_count = add_ue_to_remove(ue_to_be_removed, removed_ue_count, ue_context_p);
    ...
    if (ue_context_p->ue_context.handover_info != NULL) {
      if (ue_context_p->ue_context.handover_info->state == HO_RELEASE) {
        removed_ue_count = add_ue_to_remove(ue_to_be_removed, removed_ue_count, ue_context_p);
        rrc_gNB_handover_ue_context_release(ctxt_pP, ue_context_p);
        break; //break RB_FOREACH (why to break ?)
      }

      if (ue_context_p->ue_context.handover_info->state == HO_CANCEL) {
        rrc_gNB_handover_cancel(ctxt_pP, ue_context_p);
        ...
        if ((rrc_release_info.RRC_release_ctrl[release_num].flag > 2) &&
          ...
          if (EPC_MODE_ENABLED && !NODE_IS_DU(RC.rrc[ctxt_pP->module_id]->node_type)) {
            if (rrc_release_info.RRC_release_ctrl[release_num].flag == 4) { // if timer_s1 == 0
              rrc_gNB_send_S1AP_UE_CONTEXT_RELEASE_CPLT(ctxt_pP->module_id,
                  ue_context_p->ue_context.gNB_ue_s1ap_id);
            }

            rrc_gNB_send_GTPV1U_ENB_DELETE_TUNNEL_REQ(ctxt_pP->module_id,
                ue_context_p);

            // erase data of GTP tunnels in UE context
            for (int e_rab = 0; e_rab < ue_context_p->ue_context.nb_of_e_rabs; e_rab++) {
              ue_context_p->ue_context.gnb_gtp_teid[e_rab] = 0;
              memset(&ue_context_p->ue_context.gnb_gtp_addrs[e_rab],
                     0, sizeof(ue_context_p->ue_context.gnb_gtp_addrs[e_rab]));
              ue_context_p->ue_context.gnb_gtp_ebi[e_rab]  = 0;
            }

            struct rrc_ue_s1ap_ids_s *rrc_ue_s1ap_ids = NULL;

            rrc_ue_s1ap_ids = rrc_gNB_S1AP_get_ue_ids(RC.rrc[ctxt_pP->module_id], 0,
                              ue_context_p->ue_context.gNB_ue_s1ap_id);
  ...
  for (cur_ue = 0; cur_ue < removed_ue_count; cur_ue++) {
    ...
    rrc_gNB_free_UE(ctxt_pP->module_id, ue_to_be_removed[cur_ue]);
  ...
  RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(RC.rrc[ctxt_pP->module_id]->rrc_ue_head)) {
    ...
    if ((current_timestamp_ms - ref_timestamp_ms > RC.rrc[ctxt_pP->module_id]->aggregation_period_ms) &&
      ...
      LOG_D(LOCALIZE, "RRC [UE/id %d -> gNB/id %d] timestamp %d frame %d estimated r = %f\n",
            ctxt.rnti,
            ctxt_pP->module_id,
...
void rrc_gNB_process_AdditionResponseInformation(const module_id_t gnb_mod_idP, x2ap_ENDC_sgnb_addition_req_ACK_t *m) {
  ...
  int size;
  ue_context = rrc_gNB_get_ue_context(RC.rrc[gnb_mod_idP], m->rnti);
  ...
  PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt,
                                ...
                                m->rnti,
                                0, 0);
  size = rrc_gNB_generate_RRCReconfiguration_endc(&ctxt, ue_context, buffer, 8192, scg_CellGroupConfig, nr1_conf);
  rrc_data_req(&ctxt,
               DCCH,
               rrc_gNB_mui++,
               SDU_CONFIRM_NO,
...
//-----------------------------------------------------------------------------
void *rrc_gnb_process_itti_msg(void *notUsed) {
  MessageDef                         *msg_p;
  ...
  switch (ITTI_MSG_ID(msg_p)) {
    ...
    case RRC_MAC_CCCH_DATA_IND:
      PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt,
                                    RRC_MAC_CCCH_DATA_IND(msg_p).gnb_index,
                                    ENB_FLAG_YES,
    ...
    case S1AP_DOWNLINK_NAS:
      rrc_gNB_process_S1AP_DOWNLINK_NAS(msg_p, msg_name_p, instance, &rrc_gNB_mui);
      break;

    case S1AP_INITIAL_CONTEXT_SETUP_REQ:
      rrc_gNB_process_S1AP_INITIAL_CONTEXT_SETUP_REQ(msg_p, msg_name_p, instance);
      break;

    case S1AP_UE_CTXT_MODIFICATION_REQ:
      rrc_gNB_process_S1AP_UE_CTXT_MODIFICATION_REQ(msg_p, msg_name_p, instance);
      break;

    case S1AP_PAGING_IND:
      LOG_D(RRC, "[gNB %d] Received Paging message from S1AP: %s\n", instance, msg_name_p);
      rrc_gNB_process_PAGING_IND(msg_p, msg_name_p, instance);
      break;

    case S1AP_E_RAB_SETUP_REQ:
      rrc_gNB_process_S1AP_E_RAB_SETUP_REQ(msg_p, msg_name_p, instance);
      LOG_D(RRC, "[gNB %d] Received the message %s\n", instance, msg_name_p);
      break;

    case S1AP_E_RAB_MODIFY_REQ:
      rrc_gNB_process_S1AP_E_RAB_MODIFY_REQ(msg_p, msg_name_p, instance);
      break;

    case S1AP_E_RAB_RELEASE_COMMAND:
      rrc_gNB_process_S1AP_E_RAB_RELEASE_COMMAND(msg_p, msg_name_p, instance);
      break;

    case S1AP_UE_CONTEXT_RELEASE_REQ:
      rrc_gNB_process_S1AP_UE_CONTEXT_RELEASE_REQ(msg_p, msg_name_p, instance);
      break;

    case S1AP_UE_CONTEXT_RELEASE_COMMAND:
      rrc_gNB_process_S1AP_UE_CONTEXT_RELEASE_COMMAND(msg_p, msg_name_p, instance);
      break;

    case GTPV1U_ENB_DELETE_TUNNEL_RESP: {
      rrc_gNB_ue_context_t *ue = rrc_gNB_get_ue_context(RC.rrc[instance], GTPV1U_ENB_DELETE_TUNNEL_RESP(msg_p).rnti);
    ...
    case S1AP_PATH_SWITCH_REQ_ACK:
      LOG_I(RRC, "[gNB %d] received path switch ack %s\n", instance, msg_name_p);
      rrc_gNB_process_S1AP_PATH_SWITCH_REQ_ACK(msg_p, msg_name_p, instance);
      break;

    case X2AP_SETUP_REQ:
      rrc_gNB_process_x2_setup_request(instance, &X2AP_SETUP_REQ(msg_p));
      break;

    case X2AP_SETUP_RESP:
      rrc_gNB_process_x2_setup_response(instance, &X2AP_SETUP_RESP(msg_p));
      break;

    case X2AP_HANDOVER_REQ:
      LOG_I(RRC, "[gNB %d] target gNB Receives X2 HO Req %s\n", instance, msg_name_p);
      rrc_gNB_process_handoverPreparationInformation(instance, &X2AP_HANDOVER_REQ(msg_p));
      break;
    ...
    case X2AP_HANDOVER_REQ_ACK: {
      ...
      ue_context_p = rrc_gNB_get_ue_context(RC.rrc[instance], X2AP_HANDOVER_REQ_ACK(msg_p).rnti);
      ...
      LOG_I(RRC, "[gNB %d] source gNB receives the X2 HO ACK %s\n", instance, msg_name_p);
      ...
      if (hash_rc == HASH_TABLE_KEY_NOT_EXISTS) {
        ...
        for(int i=0; i< nb_e_rabs_tobesetup; i++) {
          ...
          ue_context_p->ue_context.gnb_gtp_x2u_ebi[i] = eps_bearer_id;
          ue_context_p->ue_context.gnb_gtp_x2u_teid[i] = x2ap_handover_req_ack->e_rabs_tobesetup[i].gtp_teid;
          gtpv1u_ue_data_p->bearers[eps_bearer_id - GTPV1U_BEARER_OFFSET].teid_tgNB = x2ap_handover_req_ack->e_rabs_tobesetup[i].gtp_teid;

          if ((x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr.length == 4) ||
              (x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr.length == 20)) {
            in_addr = *((in_addr_t *)x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr.buffer);
            ip_offset = 4;
            gtpv1u_ue_data_p->bearers[eps_bearer_id - GTPV1U_BEARER_OFFSET].tgnb_ip_addr = in_addr;
            ue_context_p->ue_context.gnb_gtp_x2u_addrs[i] = x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr;
          }

          if ((x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr.length == 16) ||
              (x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr.length == 20)) {
            memcpy(gtpv1u_ue_data_p->bearers[eps_bearer_id - GTPV1U_BEARER_OFFSET].tgnb_ip6_addr.s6_addr,
                   &x2ap_handover_req_ack->e_rabs_tobesetup[i].gNB_addr.buffer[ip_offset],
                   16);
          }
        }
      }

      rrc_gNB_process_handoverCommand(instance, ue_context_p, &X2AP_HANDOVER_REQ_ACK(msg_p));
    ...
    case X2AP_UE_CONTEXT_RELEASE: {
      struct rrc_gNB_ue_context_s        *ue_context_p = NULL;
      ue_context_p = rrc_gNB_get_ue_context(RC.rrc[instance], X2AP_UE_CONTEXT_RELEASE(msg_p).rnti);
      LOG_I(RRC, "[gNB %d] source gNB receives the X2 UE CONTEXT RELEASE %s\n", instance, msg_name_p);
      DevAssert(ue_context_p != NULL);
    ...
    case X2AP_HANDOVER_CANCEL: {
      ...
      ue_context_p = rrc_gNB_get_ue_context(RC.rrc[instance], X2AP_HANDOVER_CANCEL(msg_p).rnti);

      if (ue_context_p != NULL &&
          ue_context_p->ue_context.handover_info != NULL) {
        LOG_I(RRC, "[gNB %d] gNB receives X2 HANDOVER CANCEL for rnti %x, cause %s [%s]\n",
              instance,
        ...
        LOG_W(RRC, "[gNB %d] cannot process (%s) X2 HANDOVER CANCEL for rnti %x, cause %s, ignoring\n",
    ...
    case X2AP_ENDC_SGNB_ADDITION_REQ_ACK: {
      rrc_gNB_process_AdditionResponseInformation(ENB_INSTANCE_TO_MODULE_ID(instance), &X2AP_ENDC_SGNB_ADDITION_REQ_ACK(msg_p));
      break;
    }

    /* Messages from gNB app */
    case RRC_CONFIGURATION_REQ:
      LOG_I(RRC, "[gNB %d] Received %s : %p\n", instance, msg_name_p, &RRC_CONFIGURATION_REQ(msg_p));
      openair_rrc_gNB_configuration(ENB_INSTANCE_TO_MODULE_ID(instance), &RRC_CONFIGURATION_REQ(msg_p));
      break;
    ...
    case M2AP_SETUP_RESP:
      rrc_gNB_process_M2AP_SETUP_RESP(&ctxt,0/*CC_id*/,ENB_INSTANCE_TO_MODULE_ID(instance),&M2AP_SETUP_RESP(msg_p));
      break;

    case M2AP_MBMS_SCHEDULING_INFORMATION:
      rrc_gNB_process_M2AP_MBMS_SCHEDULING_INFORMATION(&ctxt,0/*CC_id*/,ENB_INSTANCE_TO_MODULE_ID(instance),&M2AP_MBMS_SCHEDULING_INFORMATION(msg_p));
      break;

    case M2AP_MBMS_SESSION_START_REQ:
      rrc_gNB_process_M2AP_MBMS_SESSION_START_REQ(&ctxt,0/*CC_id*/,ENB_INSTANCE_TO_MODULE_ID(instance),&M2AP_MBMS_SESSION_START_REQ(msg_p));
      break;

    case M2AP_MBMS_SESSION_STOP_REQ:
      rrc_gNB_process_M2AP_MBMS_SESSION_STOP_REQ(&ctxt,&M2AP_MBMS_SESSION_STOP_REQ(msg_p));
      break;

    case M2AP_RESET:
      rrc_gNB_process_M2AP_RESET(&ctxt,&M2AP_RESET(msg_p));
      break;

    case M2AP_ENB_CONFIGURATION_UPDATE_ACK:
      rrc_gNB_process_M2AP_ENB_CONFIGURATION_UPDATE_ACK(&ctxt,&M2AP_ENB_CONFIGURATION_UPDATE_ACK(msg_p));
      break;

    case M2AP_ERROR_INDICATION:
      rrc_gNB_process_M2AP_ERROR_INDICATION(&ctxt,&M2AP_ERROR_INDICATION(msg_p));
      break;

    case M2AP_MBMS_SERVICE_COUNTING_REQ:
      rrc_gNB_process_M2AP_MBMS_SERVICE_COUNTING_REQ(&ctxt,&M2AP_MBMS_SERVICE_COUNTING_REQ(msg_p));
      break;

    case M2AP_MCE_CONFIGURATION_UPDATE:
       rrc_gNB_process_M2AP_MCE_CONFIGURATION_UPDATE(&ctxt,&M2AP_MCE_CONFIGURATION_UPDATE(msg_p));
       break;
    ...
    default:
      LOG_E(RRC, "[gNB %d] Received unexpected message %s\n", instance, msg_name_p);
      break;
  }
...
//-----------------------------------------------------------------------------
{
  ...
  while (1) {
    (void) rrc_gnb_process_itti_msg(NULL);
    {
...
rrc_gNB_process_SidelinkUEInformation(
  const protocol_ctxt_t *const ctxt_pP,
  rrc_gNB_ue_context_t         *ue_context_pP,
  NR_SidelinkUEInformation_r12_t *sidelinkUEInformation
)
//-----------------------------------------------------------------------------
{
  NR_SL_DestinationInfoList_r12_t  *destinationInfoList;
  ...
  if (sidelinkUEInformation->criticalExtensions.present == NR_SidelinkUEInformation_r12__criticalExtensions_PR_c1) {
    if (sidelinkUEInformation->criticalExtensions.choice.c1.present == NR_SidelinkUEInformation_r12__criticalExtensions__c1_PR_sidelinkUEInformation_r12) {
      ...
      if ((sidelinkUEInformation->criticalExtensions.choice.c1.choice.sidelinkUEInformation_r12.commTxResourceReq_r12 != NULL) &&
        ...
        destinationInfoList = CALLOC(1, sizeof(NR_SL_DestinationInfoList_r12_t));
        ...
        rrc_gNB_generate_RRCReconfiguration_Sidelink(ctxt_pP, ue_context_pP, destinationInfoList, 0);
      ...
      if (sidelinkUEInformation->criticalExtensions.choice.c1.choice.sidelinkUEInformation_r12.discTxResourceReq_r12 != NULL) {
        ...
        //generate RRC Reconfiguration
        rrc_gNB_generate_RRCReconfiguration_Sidelink(ctxt_pP, ue_context_pP, NULL, n_discoveryMessages);
        return 0;
      }
...
//-----------------------------------------------------------------------------
int
rrc_gNB_generate_RRCReconfiguration_Sidelink(
  const protocol_ctxt_t *const ctxt_pP,
  rrc_gNB_ue_context_t *const ue_context_pP,
  NR_SL_DestinationInfoList_r12_t  *destinationInfoList,
...
//-----------------------------------------------------------------------------
{
  ...
  if ((destinationInfoList != NULL) && (destinationInfoList->list.count > 0)) {
    LOG_I(RRC,"[gNB %d] Frame %d, Generate NR_RRCReconfiguration_Sidelink (bytes %d, UE id %x), number of destinations %d\n",
    ...
    NR_SL_CommConfig_r12_t  sl_CommConfig[destinationInfoList->list.count];
    //get a RP from the available RPs
    sl_CommConfig[0] = rrc_gNB_get_sidelink_commTXPool(ctxt_pP, ue_context_pP, destinationInfoList);
    size = do_RRCReconfiguration(ctxt_pP,
                                           buffer,
                                           rrc_gNB_get_next_transaction_identifier(ctxt_pP->module_id),   //Transaction_id
                                           (NR_SRB_ToAddModList_t *)NULL,
                                           ...
                                           NULL, NULL,  NULL, NULL, NULL, NULL, NULL,
                                           (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *)NULL,
                                           (NR_SL_CommConfig_r12_t *)&sl_CommConfig,
                                           (NR_SL_DiscConfig_r12_t *)NULL,
                                           (NR_SCellToAddMod_r10_t *)NULL
                                          );
    //
  ...
  if (n_discoveryMessages > 0) {
    NR_SL_DiscConfig_r12_t sl_DiscConfig[n_discoveryMessages];
    //get a RP from the available RPs
    sl_DiscConfig[0] = rrc_gNB_get_sidelink_discTXPool(ctxt_pP, ue_context_pP, n_discoveryMessages );
    size = do_RRCReconfiguration(ctxt_pP,
                                           buffer,
                                           rrc_gNB_get_next_transaction_identifier(ctxt_pP->module_id),   //Transaction_id
                                           (NR_SRB_ToAddModList_t *)NULL,
                                           ...
                                           (struct NR_RRCReconfiguration_r8_IEs__dedicatedInfoNASList *)NULL,
                                           (NR_SL_CommConfig_r12_t *)NULL,
                                           (NR_SL_DiscConfig_r12_t *)&sl_DiscConfig,
                                           (NR_SCellToAddMod_r10_t *)NULL
                                          );
  }

  LOG_I(RRC,"[gNB %d] Frame %d, Logical Channel DL-DCCH, Generate NR_RRCReconfiguration_Sidelink (bytes %d, UE id %x)\n",
        ctxt_pP->module_id,ctxt_pP->frame, size, ue_context_pP->ue_context.rnti);
  rrc_data_req(
    ctxt_pP,
    DCCH,
...
NR_SL_CommConfig_r12_t rrc_gNB_get_sidelink_commTXPool( const protocol_ctxt_t *const ctxt_pP, rrc_gNB_ue_context_t *const ue_context_pP, NR_SL_DestinationInfoList_r12_t  *destinationInfoList ) {
  // for the moment, use scheduled resource allocation
  NR_SL_CommConfig_r12_t sl_CommConfig_r12;
  NR_SL_CommConfig_r12_t  *sl_CommConfig = &sl_CommConfig_r12;
  NR_SL_CommResourcePool_r12_t    *sc_CommTxConfig;
  memset(sl_CommConfig,0,sizeof(NR_SL_CommConfig_r12_t));
  sl_CommConfig->commTxResources_r12 = CALLOC(1, sizeof(*sl_CommConfig->commTxResources_r12));
  sl_CommConfig->commTxResources_r12->present = NR_SL_CommConfig_r12__commTxResources_r12_PR_setup;
  sl_CommConfig->commTxResources_r12->choice.setup.present = NR_SL_CommConfig_r12__commTxResources_r12__setup_PR_scheduled_r12;
  ...
  sl_CommConfig->commTxResources_r12->choice.setup.choice.scheduled_r12.mac_MainConfig_r12.retx_BSR_TimerSL = NR_RetxBSR_Timer_r12_sf320; //MacConfig, for testing only
  ...
  sc_CommTxConfig->sc_CP_Len_r12 = NR_SL_CP_Len_r12_normal;
  sc_CommTxConfig->sc_Period_r12 = NR_SL_PeriodComm_r12_sf40;
  sc_CommTxConfig->data_CP_Len_r12 = NR_SL_CP_Len_r12_normal;
  ...
  sc_CommTxConfig->sc_TF_ResourceConfig_r12.offsetIndicator_r12.present = NR_SL_OffsetIndicator_r12_PR_small_r12;
  sc_CommTxConfig->sc_TF_ResourceConfig_r12.offsetIndicator_r12.choice.small_r12 = 0;
  sc_CommTxConfig->sc_TF_ResourceConfig_r12.subframeBitmap_r12.present = NR_SubframeBitmapSL_r12_PR_bs40_r12;
  ...
  sc_CommTxConfig->dataHoppingConfig_r12.numSubbands_r12  = NR_SL_HoppingConfigComm_r12__numSubbands_r12_ns1;
  ...
  sc_CommTxConfig->ue_SelectedResourceConfig_r12->data_TF_ResourceConfig_r12.offsetIndicator_r12.present = NR_SL_OffsetIndicator_r12_PR_small_r12;
  sc_CommTxConfig->ue_SelectedResourceConfig_r12->data_TF_ResourceConfig_r12.offsetIndicator_r12.choice.small_r12 = 0 ;
  sc_CommTxConfig->ue_SelectedResourceConfig_r12->data_TF_ResourceConfig_r12.subframeBitmap_r12.present = NR_SubframeBitmapSL_r12_PR_bs40_r12;
  ...
  sc_CommTxConfig->txParameters_r12->sc_TxParameters_r12.alpha_r12 = NR_Alpha_r12_al0;
...
NR_SL_DiscConfig_r12_t rrc_gNB_get_sidelink_discTXPool( const protocol_ctxt_t *const ctxt_pP, rrc_gNB_ue_context_t *const ue_context_pP,  int n_discoveryMessages ) {
  //TODO
  NR_SL_DiscConfig_r12_t  sl_DiscConfig;
  memset(&sl_DiscConfig,0,sizeof(NR_SL_DiscConfig_r12_t));
  sl_DiscConfig.discTxResources_r12 = CALLOC(1,sizeof(*sl_DiscConfig.discTxResources_r12));
  sl_DiscConfig.discTxResources_r12->present = NR_SL_DiscConfig_r12__discTxResources_r12_PR_setup;
  sl_DiscConfig.discTxResources_r12->choice.setup.present = NR_SL_DiscConfig_r12__discTxResources_r12__setup_PR_scheduled_r12;
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      /*
          ...
          case GTPV1U_ENB_DELETE_TUNNEL_RESP:
            ...
            //LOG_I(RRC, "[gNB %d] Received message %s, not processed because procedure not synched\n",
            //instance, msg_name_p);
            if (rrc_gNB_get_ue_context(RC.nrrrc[instance], GTPV1U_ENB_DELETE_TUNNEL_RESP(msg_p).rnti)
                && rrc_gNB_get_ue_context(RC.nrrrc[instance], GTPV1U_ENB_DELETE_TUNNEL_RESP(msg_p).rnti)->ue_context.ue_release_timer_rrc > 0) {
              rrc_gNB_get_ue_context(RC.nrrrc[instance], GTPV1U_ENB_DELETE_TUNNEL_RESP(msg_p).rnti)->ue_context.ue_release_timer_rrc =
              rrc_gNB_get_ue_context(RC.nrrrc[instance], GTPV1U_ENB_DELETE_TUNNEL_RESP(msg_p).rnti)->ue_context.ue_release_timer_thres_rrc;
            }
            break;
