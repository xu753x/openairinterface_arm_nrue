                             ...
                             const nfapi_nr_uci_pucch_pdu_format_0_1_t *uci_01)
{
  ...
  if (((uci_01->pduBitmap >> 1) & 0x1) || sched_ctrl->SR) {
    if (uci_01->harq != NULL && uci_01->harq->harq_confidence_level==0) sched_ctrl->tpc1 = nr_get_tpc(RC.nrmac[mod_id]->pucch_target_snrx10, uci_01->ul_cqi, 30);
...
int ind_found=-1;
  ...
  for (int i=0;i<8;i++) {
    ...
    if (RC.nrmac[mod_id]->flexible_slots_per_frame[pucch_slot%n_slots_frame] > 0) {
