...
NR_UE_L2_STATE_t nr_ue_scheduler(nr_downlink_indication_t *dl_info, nr_uplink_indication_t *ul_info){
  ...
  if (nr_update_bsr(mod_id, txFrameP, txSlotP, eNB_indexP) == TRUE) {
...
nr_update_bsr(module_id_t module_idP, frame_t frameP,
           slot_t slotP, eNB_index_t eNB_index) {
  ...
  for (lcid=DCCH; lcid < NR_MAX_NUM_LCID; lcid++) {
    ...
    if (1) {
      ...
      rlc_status = mac_rlc_status_ind(module_idP, mac->crnti,eNB_index,frameP,slotP,ENB_FLAG_NO,MBMS_FLAG_NO,
                                      lcid,
                                      0, 0
                                     );
      //rlc_buffer_occupancy_t lcid_buffer_occupancy_new = mac_rlc_get_buffer_occupancy_ind(module_idP, mac->crnti, eNB_index, frameP, slotP, ENB_FLAG_NO, lcid);
  ...
  if (mac->BSR_reporting_active) {
    ...
    if (buflen >= 4) {
      ...
      if (num_lcg_id_with_data <= 1) {
    	...
    	NR_BSR_LONG_SIZE = num_lcg_id_with_data + 1;
        bsr_ce_len = NR_BSR_LONG_SIZE;
    ...
    if(lcid_buffer_occupancy_new) {
      buflen_remain =
          buflen - (bsr_len + phr_len +
        		  total_rlc_pdu_header_len + sdu_length_total + MAX_RLC_SDU_SUBHEADER_SIZE);
//        		  total_rlc_pdu_header_len + sdu_length_total + sizeof(SCH_SUBHEADER_FIXED));
      LOG_D(NR_MAC,
      ...
      while ((!is_lcid_processed) && (lcid_buffer_occupancy_new)
        ...
        buflen_remain = buflen - (bsr_len + phr_len + total_rlc_pdu_header_len + sdu_length_total + MAX_RLC_SDU_SUBHEADER_SIZE);
        //buflen_remain = buflen - (bsr_len + phr_len + total_rlc_pdu_header_len + sdu_length_total + sizeof(SCH_SUBHEADER_FIXED));
      }

      //Update Buffer remain and BSR bytes after transmission
      mac->scheduling_info.LCID_buffer_remain[lcid] = lcid_buffer_occupancy_new;
      mac->scheduling_info.BSR_bytes[mac->scheduling_info.LCGID[lcid]] += (lcid_buffer_occupancy_new - lcid_buffer_occupancy_old);
      LOG_D(MAC,
...
#endif
  LOG_T(MAC, "[UE %d] Frame %d: bsr s %p bsr_l %p\n",
        module_idP, frameP, bsr_s, bsr_l);
  ...
  if ((padding_len) && (bsr_len == 0)) {
    ...
    if (padding_len >= (1 + NR_BSR_LONG_SIZE)) {
      bsr_ce_len = NR_BSR_LONG_SIZE;
      bsr_header_len = 1;
      // Trigger BSR Padding
      mac->BSR_reporting_active |=
