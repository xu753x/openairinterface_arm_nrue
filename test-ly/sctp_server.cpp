...
*/
  ...
  int sctp_server::create_socket (const char * address, const uint16_t port_num){
    if ((socket_ = socket (AF_INET6, SOCK_STREAM, IPPROTO_SCTP)) < 0) {
      printf("socket: %s:%d",strerror(errno),errno);
      return -1;
    }
    printf("Created socket(%d)",socket_);
    ...
    if(bind(socket_,(struct sockaddr *)&serverAddr_,sizeof(serverAddr_)) != 0){
      printf("socket bind: %s:%d",strerror(errno),errno);
    }
    ...
    printf("[%d] Sending explicit connect to 172.17.226.175:%u\n", socket_, 38412);
    /*
     * Connect to remote host and port
     */
    if (sctp_connectx (socket_, (struct sockaddr *)&serverAddr_, 1, NULL) < 0) {
      printf("Connect to 172.17.226.175:%u failed: %s\n", 38412, strerror (errno));
    }
    listen(socket_,5);
  ...
  void sctp_server::start_receive(sctp_application *app){
    ...
    uint8_t buffer[1000];
    int encoded_size = ng->encode2buffer(buffer, 1000);
    sctp_sendmsg (socket_, (const void *)buffer, encoded_size, NULL, 0, htonl(app_->getPpid()), 0, 0, 0, 0);
    while(true ){
	sctp_read_from_socket(socket_,app_->getPpid());
  }
  ...
  void  *sctp_server::sctp_receiver_thread(void * arg){
    ...
    printf("-----------NGAP Create pthread to receive sctp message\n----------------------");
    int fdmax;
    int clientsock;
    fd_set master;
    fd_set read_fds;
    ...
    printf("-----------NGAP  debug 2\n----------------------");
      ...
      if (select (fdmax + 1, &read_fds, NULL, NULL, NULL) == -1) {
        printf("[socket(%d)] Select() error: %s:%d",ptr->getSocket(),strerror(errno),errno);
        pthread_exit (NULL);
      }
        ...
        if (FD_ISSET (i, &read_fds)) {
          if (i == ptr->getSocket()) {
            if ((clientsock = accept (ptr->getSocket(), NULL, NULL)) < 0) {
              printf("[socket(%d)] accept() error: %s:%d",ptr->getSocket(),strerror(errno),errno);
              pthread_exit (NULL);
            }else{
  ...
  int sctp_server::sctp_read_from_socket(int sd, uint32_t ppid){
    ...
    //if(n < 0){
     // printf("sctp_recvmsg error:: %s:%d",strerror(errno),errno);
    ...
    if (flags & MSG_NOTIFICATION) {
      ...
      sctp_association_t              *association;
       ...
       // if (ntohl (sinfo.sinfo_ppid) != association->ppid) {
         // printf("Received data from peer with unsollicited PPID(%d), expecting(%d)",ntohl (sinfo.sinfo_ppid), association->ppid);
         // return SCTP_RC_ERROR;
       // }
        printf("[assoc_id(%d)][socket(%d)] Msg of length(%d) received from port(%d), on stream(%d), PPID(%d)\n",sinfo.sinfo_assoc_id, sd, n, ntohs (addr.sin6_port), sinfo.sinfo_stream, ntohl (sinfo.sinfo_ppid));
        bstring payload = blk2bstr(buffer, n);//payload need free
        //handle payload
        printf("变量app_->getPpid()1: %d\n", app_->getPpid());
        app_->handle_receive(payload, (sctp_assoc_id_t) sinfo.sinfo_assoc_id, sinfo.sinfo_stream, 1,1);
    }
  }
    ...
    switch (sctp_assoc_changed->sac_state) {
      case SCTP_COMM_UP: {
        if (add_new_association(sd, ppid, sctp_assoc_changed) == NULL) {
          printf("add new association with ppid(%d) socket(%d) error",ppid,sd);
          rc = SCTP_RC_ERROR;
        }
        break;
      }
      ...
      default:
          printf("Logging unhandled sctp message(%d)",sctp_assoc_changed->sac_state);
        break;
    }
    return rc;
  }
  ...
  sctp_association_t* sctp_server::add_new_association(int sd, uint32_t ppid, struct sctp_assoc_change *sctp_assoc_changed) {
    ...
    printf("add new association with id(%d)",(sctp_assoc_id_t) sctp_assoc_changed->sac_assoc_id);
  ...
  int sctp_server::sctp_get_peeraddresses(int sock, struct sockaddr **remote_addr, int *nb_remote_addresses){
    ...
    if ((nb = sctp_getpaddrs (sock, -1, &temp_addr_p)) <= 0) {
        printf("Failed to retrieve peer addresses");
      return -1;
    }
    printf("----------------------");
      ...
      for(int j=0; j<nb; j++){
        if (temp_addr_p[j].sa_family == AF_INET) {
          ...
          if (inet_ntop (AF_INET6, &addr->sin6_addr.s6_addr, address, sizeof (address)) != NULL) {
            ...
            printf("    - unhandled address family %d", temp_addr_p[j].sa_family);
        }
      }
      printf("----------------------");
...
int sctp_server::getSocket(){
  ...
  int sctp_server::sctp_send_msg(sctp_assoc_id_t sctp_assoc_id, sctp_stream_id_t stream, bstring *payload){
    ...
    if ((assoc_desc = sctp_is_assoc_in_list (sctp_assoc_id)) == NULL) {
      printf("This assoc id(%d) has not been fount in list", sctp_assoc_id);
      return -1;
    }
    if (assoc_desc->sd == -1) {
        printf("The socket is invalid may be closed (assoc id %d)", sctp_assoc_id);
      return -1;
    }
    printf("[%d][%d] Sending buffer %p of %d bytes on stream %d with ppid %d", assoc_desc->sd, sctp_assoc_id, bdata(*payload), blength(*payload), stream, assoc_desc->ppid);
    if (sctp_sendmsg (assoc_desc->sd, (const void *)bdata(*payload), (size_t) blength(*payload), NULL, 0, htonl(assoc_desc->ppid), 0, stream, 0, 0) < 0) {
        printf("send, sd:%u,stream:%u,ppid:%u, len:%u, failed: %s,%d",assoc_desc->sd,stream, htonl(assoc_desc->ppid), blength(*payload), strerror (errno), errno);
      //bstr__free((*payload)->data);
      bdestroy(*payload); 
      *payload = NULL;
      return -1;
    }
    printf("Successfully sent %d bytes on stream %d", blength(*payload), stream);
    //bstr__free((*payload)->data); 
    bdestroy(*payload); 
    *payload = NULL;
