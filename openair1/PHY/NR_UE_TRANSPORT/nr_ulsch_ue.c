...
void nr_ue_ulsch_procedures(PHY_VARS_NR_UE *UE,
  ...
  for (cwd_index = 0;cwd_index < num_of_codewords; cwd_index++) {
    ...
    LOG_I(PHY,"ulsch %x : frame %d %d start_rb %d %d, bwp_start %d %d start_sc %d start_symbol %d num_symbols %d cdmgrpsnodata %d num_dmrs %d dmrs_re_per_rb %d, pos 0x%x\n",rnti, frame,slot,
          start_rb,nb_rb, pusch_pdu->bwp_start,pusch_pdu->bwp_size, start_sc,start_symbol,number_of_symbols,cdm_grps_no_data,number_dmrs_symbols,nb_dmrs_re_per_rb, ul_dmrs_symb_pos);
